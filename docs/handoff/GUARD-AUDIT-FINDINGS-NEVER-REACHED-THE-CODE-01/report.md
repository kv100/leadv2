...
## INHERITED-UNVERIFIABLE-VERDICTS-RECHECK-01 — recheck of every «UNVERIFIABLE» in this file

Четыре вердикта помечены `UNVERIFIABLE` в этом файле (не считая рекомендации 2
`PROCESS-AUDIT-20260821`, уже исправленной выше на APPLIED через `REVIEW-UNION-VERDICT-01`).
Перепроверила каждый живым способом, не чтением.

**R4 «native cache probe» / R5 «ask Z.AI» (GLM-EFFICIENCY-AUDIT-01)** — грепнула оба репозитория
(`leadv2` и `persona-engine`, `docs/handoff/`) по `cached_tokens|api/paas/v4` — совпадения только
в шумных `developer.stream.jsonl` (не пробы, пустой контекст вокруг совпадения). Артефакта пробы
нет нигде. **R4 переклассифицирую: не «неверифицируемо», а НЕИЗМЕРЕНО** — критерий выполнения
формулируется прямо: "выполнено, когда в дереве есть артефакт с двумя вызовами native
`api/paas/v4` с одинаковым промптом и напечатанным `cached_tokens` из обоих ответов". Это
конкретная ручная проба, никто её не запускал. **R5 — другое: закрываю как НЕ-ИЗМЕРИМОЕ по
определению**, рассуждение: вопрос вендору не имеет file:line или лог-строки в этом репозитории
в принципе — ответ живёт либо в переписке с Z.AI, либо нигде. Если ответ придёт, он ляжет вне
кода (документ/память), не как код-факт.

**R6 `[1m]` trial (GLM-EFFICIENCY-AUDIT-01)** — грепнула `CLAUDE_CODE_AUTO_COMPACT_WINDOW` и
`glm-5.3[1m]`/`[1m]` по `plugins/leadv2/scripts/*.sh` и `config/*.yaml` — **0 совпадений**,
подтверждено, не применено. **Переклассифицирую: НЕИЗМЕРЕНО** (не «неверифицируемо») — критерий
прост: "выполнено, когда `GLM_MODEL` для одной Heavy-линии содержит `[1m]` И
`CLAUDE_CODE_AUTO_COMPACT_WINDOW=1000000` в её окружении, с записанным до/после сравнением".

**Required-status-checks (FIVE-DAY-AUDIT-BEFORE-STATE-OWNER-01, строка 3)** — «UNVERIFIABLE
offline» была НЕОБЯЗАТЕЛЬНОЙ формулировкой: `gh` авторизован в этой среде (не offline).
`gh api repos/kv100/leadv2/branches/main/protection` → `404 Not Found`. НО: активный gh-аккаунт
на этот репозиторий имеет только `pull: true` (`admin: false, push: false`) — GitHub отдаёт 404
на этот эндпоинт **и** при отсутствии правила, **и** при недостаточном доступе, поэтому 404 сам
по себе не различает два случая. Честный вывод: **не «offline», а «недостаточно прав у
текущего gh-токена»** — конкретная, отличная от исходной причина неопределённости. Критерий
закрытия: "выполнено, когда `gh api .../protection` отвечает 200 с телом ИЛИ 404, подтверждённый
токеном с `admin:true` на этот репозиторий".

### Итог переклассификации
Из пяти вердиктов `UNVERIFIABLE` в этом файле: 1 (рекомендация 2 PROCESS-AUDIT) была ЛОЖНОЙ,
теперь APPLIED; 1 (R5, вопрос вендору) закрыт как честно не-измеримое; 3 (R4, R6,
required-status-checks) переклассифицированы в НЕИЗМЕРЕНО с записанным критерием каждое — заведены
отдельными строками бэклога, не растворены здесь.
