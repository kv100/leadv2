...
| 5fa969ac | слита ОТДЕЛЬНОЙ линией `RECOVER-5FA969AC-01` | — (см. её отчёт) |
| BROAD-STATUS-READY-FIRES-ON-A-DAY-OLD-FILE-01 (аддендум) | слита | — |
| SMART-ARBITER-01 (аддендум) | уже была слита, `b718c1ec` | b718c1ec |

## ЗАКРЫТО 2026-09-05 — все строки в main, проверено поимённо

Проверка сделана лидом не по этой таблице, а независимо от неё: по каждой оставшейся ветке
`git merge-base --is-ancestor <branch> main`. Обе оставшиеся — `worktree-5fa969ac` и
`worktree-BROAD-STATUS-READY-FIRES-ON-A-DAY-OLD-FILE-01` — дали положительный ответ.
Таблица выше на момент проверки была УСТАРЕВШЕЙ (`5fa969ac | см. ниже`), то есть отчёт
занижал результат: строка ушла в собственную линию и там закрылась, а сюда это не вернулось.

Что эта запись НЕ утверждает: что содержимое слияний семантически правильно. Проверено только
попадание в main по имени ветки. Семантику скажут сюиты, а не этот абзац.

**Незакрытый хвост, перенесённый в реестр отложенных решений** (`SD-NO-WORK-TERMINAL-FOUR-RED-01`):
`test-no-work-terminal.sh` несёт 4 приколоченно-красных проверки, они были красными на кончике
самой мёртвой ветки, и починка FP-08 (унификация ожидания), которая их удовлетворяет, — это
недоделанная работа, которую восстановление СОХРАНИЛО, но не доделало. Источник — §7 отчёта
`RECOVER-5FA969AC-01`. Красная сюита без строки в реестре через сутки читается как «так и было».
