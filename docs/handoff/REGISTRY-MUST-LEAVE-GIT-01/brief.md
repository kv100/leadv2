...
- ~~`leadv2_active_unregister` возвращает rc=0 и не меняет ничего~~ — **это НЕ следствие
  git-копий, поправка ведущей 2026-09-04T04:40Z.** У `leadv2-active-registry.sh` вообще нет
  CLI: файл заканчивается определением функции, диспетчера argv в нём нет. Поэтому любой вызов
  вида `bash leadv2-active-registry.sh unregister <id>` — молчаливый no-op с кодом 0, и
  прежний замер (шесть вызовов, было 9 строк, стало 9) объясняется именно этим. Через `source`
  плюс `leadv2_active_unregister <id>` (task_id ПОЗИЦИОННО, не `--task-id`) чистка сработала
  сразу: 6 строк → 0. Симптом настоящий, но приписан был не той причине;

  **Отдельный дефект, вскрытый той же поправкой:** скрипт молча глотает и неизвестный флаг
  (`--task-id X` вместо позиционного), и прямой запуск без диспетчера — оба пути возвращают 0.
  Ноль от операции, которая ничего не сделала, неотличим от нуля от операции, которая сделала.
  Почини это вместе с путём состояния: либо дай файлу настоящий CLI, либо пусть прямой запуск
  падает с внятной строкой «этот файл нужно сорсить»;
