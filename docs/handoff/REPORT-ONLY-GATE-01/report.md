...
Conflict-resolution note: the findings-rendering side contributed the appended rendered findings block, atomic `review-gate.md.tmp` then `mv -f` publication, and advisory `do_not_merge=1` terminal evidence; the report-deliverable side contributed the report-kind PASS head (`kind`, `deliverable`, `bytes`, and `review`) plus the harvested deliverable passed through the five-argument terminal note. The merged PASS exit preserves both contributions by sharing the renderer and atomic publication across report and diff branches while keeping each branch’s terminal-note arity.
