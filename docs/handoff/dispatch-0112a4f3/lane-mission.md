Product implementation task dispatch-0112a4f3. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CODEX-LEAD-PLUGIN-01 — package the Codex lead as a REAL Codex plugin (founder order: полноценно, не пилот)

Repo: ~/Projects/leadv2. Context: plugins/leadv2/codex-lead/ (prompt pack, lv2guard.sh,
leadv2-codex-status.sh, install.sh — merged today as CODEX-LEAD-FULL-01). The founder was right
and the prior assumption wrong: Codex HAS a plugin system since 2026-03 (marketplace, skills,
MCP servers, hooks). Local CLI codex-cli 0.145.0-alpha.1 confirms:
`codex plugin add|list|remove`, `codex plugin marketplace add <local path|git>`.

Known on-disk format (from openai/codex docs — VERIFY empirically against the local CLI, it is
alpha and authoritative):
- plugin root: `plugin.json` manifest (metadata + capability declarations)
- `skills/` dir with markdown instruction files
- MCP servers declared in plugin.json
- hooks declared in the manifest (docs mention PreToolUse interception — if the local CLI
  supports blocking hooks, wire lv2guard THERE: that upgrades the deny floor from prose to
  enforced, killing the pilot's biggest weakness)
- marketplace = a source dir/repo the CLI snapshots; install registers under [plugins] in
  config.toml

## Deliverables
1. `plugins/leadv2/codex-lead/marketplace/` — a local Codex marketplace containing one plugin
   `leadv2` : plugin.json + skills/ (converted from the 5 prompt-pack files; keep $ARGUMENTS
   semantics if skills support args, else embed usage lines) + repowise MCP declaration +
   hooks (lv2guard as PreToolUse) IF the local CLI's manifest schema accepts them — probe,
   don't assume; if hooks are unsupported/advisory in 0.145.0-alpha.1, say so in the summary
   with the probe output and keep lv2guard prose-mandated.
2. Empirical install proof (EVIDENCE CONTRACT): `codex plugin marketplace add <path>` +
   `codex plugin add leadv2` + `codex plugin list` outputs captured in the summary; then
   `codex plugin remove` to leave the host clean (install is the founder's call).
3. Update install.sh: prefer the plugin path (marketplace add + plugin add) with the old
   prompts-copy as fallback for CLIs without plugin support; idempotent both ways.
4. Update docs/codex-lead-pilot-runbook.md: replace pilot framing with the full-implementation
   install (2 commands), keep the measurement checklist section but note which gates became
   ENFORCED via plugin hooks vs remain prose.
5. Tests: manifest JSON validates (python3 json.load), skills present, install.sh idempotency
   still green; bash -n everything.

Constraints: never edit outside ~/Projects/leadv2; do not install the plugin persistently on
this host (add→verify→remove). HARD RULE: no halting on prepass/census mismatches — probe the
live CLI, one summary line per mismatch, PROCEED.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-0112a4f3" "<question>" \
