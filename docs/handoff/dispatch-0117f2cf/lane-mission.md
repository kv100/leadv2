Product implementation task dispatch-0117f2cf. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# REVIEW-ROUND1-EXHAUSTIVE-01 — ROUND 2 FINISHER (resume of lane 2a2a3fb5)

## Outcome (one)
Close the round-1 review FAIL: fix H1-H4 + M1-M2 (LOWs if trivial) from
docs/handoff/dispatch-2a2a3fb5-review/critic.full.md — read it FIRST (file:line, executed repros,
prescribed fixes; reviewer probe scripts referenced there). Then COMMIT on the lane branch.

Punch list:
1. H1 round-counter regression: monotonic REVIEW_ROUND=$((sidecar_round+1)) whenever a real prior
   verdict exists, mode-independent; findings sourced from the HIGHEST snapshot; snapshot refresh
   (overwrite when live gate differs), never `[[ -f ]] || cp`.
2. H2 prior_findings count crosses the subshell boundary (first-line count or tempfile).
3. H3 empty prior-findings body ⇒ fall back to EXHAUSTIVE (kill the operator hatch), for both
   LEADV2_REVIEW_ROUND=2 and any empty-body verify_only path.
4. H4 coverage: tests for the emit line, forced modes, round>=3 + regression repro from H1, and
   the 40-finding/300-char cap — each red-first against pinned baseline.
5. M1 empty diff-hash ⇒ exhaustive + no sidecar write, stderr logged. M2 numeric guard on
   sidecar_round.

## Off_limits
Same as round 1: only leadv2-review-run.sh + its tests (+ skills file if already in your diff);
no product-close, no arm/pool/quota logic.

## Acceptance
- Critic's H1 repro sequence (fail→fix→verify→re-review unchanged→fix) now yields monotonic
  rounds and fresh findings — run it, paste raw sidecar lines.
- All new tests red-first (raw output); existing suites green (body-persist 13, arm-no-verdict 15,
  silence-gate 15); bash -n both bashes + shellcheck rc=0; commit on lane branch.

## Rollback
Single revert.

## Terminal artifact
Commit sha + per-finding note + raw repro/test output + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-0117f2cf" "<question>" \
