Product implementation task dispatch-0a503a5e. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# PLUGIN-CORE-OFFLINE-4RED-01 — ROUND 3 FINISHER (resume of lane e2e9d9b2) — one flaky suite + commit

State: your r2 hermetization works for the original 4 suites. Lead ran the acceptance himself:
RUN1 = 49/1 (FAILED: supervisor reconciliation), RUN2 = 50/0 — ONE remaining order/state-dependent
suite. Acceptance (50/0 twice back-to-back) not yet met. Also: ALL lane work is still uncommitted
(r2 worker timed out before committing).

## Do exactly two things
1. Hermetize the "supervisor reconciliation" suite: find why it fails on the first full-runner
   pass but passes on the second (state left by an earlier suite, or state it expects pre-created:
   cache dirs, ledgers, .pulse files, registry). Isolate its state under its own TMP_ROOT the way
   your other fixes do. NEVER weaken an assertion. Falsify after fixing (break the guarded path →
   suite RED).
2. COMMIT the complete lane work (r1+r2+this) on the lane branch with a clean diff: tests/fixtures
   only; the +90 journal lines and any docs/leadv2 churn stay OUT (per r1 review MEDIUM-1).

## Acceptance
- run-core-offline.sh 50/0 TWICE back-to-back from a clean shell (raw output of both).
- git status clean outside plugins/leadv2/scripts/tests/** after the runs.
- bash -n + shellcheck; commit sha in deliverable.

## Off_limits
Everything else. No engine-code edits.

## Terminal artifact
Commit sha + root cause of the reconciliation flake (one line) + both raw run summaries +
DELIVERABLE_COMPLETE.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-0a503a5e" "<question>" \
