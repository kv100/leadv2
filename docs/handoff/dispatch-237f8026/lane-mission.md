Product implementation task dispatch-237f8026. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
## Step 0 — GATE. Do this first and STOP if it fails.

The design's whole premise is that nothing on the critical path may assume a Claude lead: Codex
must be able to run a full lead cycle, not just build or review. **Verify, do not assume**, what a
Codex-led session (`leadv2-codex-session-runner.sh`) can actually invoke:

- Can it run a plain `bash` script from the plugin's `scripts/` directory? Prove it by running one.
- Does it have the Agent tool? The Workflow tool? In-session MCP? Prove each answer.
- What is its working directory and its sandbox/permission scope for writes under
  `docs/handoff/<task>/`?

Write the answers with evidence to `docs/handoff/one-review-path-2026-08-06/codex-capability.md`.

**If a plain bash script is NOT invocable from a Codex-led session, STOP and return BLOCKED with
that evidence.** The entire design rests on the script being the canonical container; if that is
false, the design is wrong and the founder needs to know before any code is written, not after.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-237f8026" "<question>" \
