Product implementation task dispatch-24bab870. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# MISSION — GLM-FIRST-RECOVERY-01 (the router falls off GLM and never comes back)

Plugin repo: `~/Projects/leadv2`. Founder-raised 2026-08-15, added to Track 5.

## The founder's observation, which matches the logs

> "иногда через диспатч забываешь о том что глм ферст… мы делаем работу в его пик часы, ты верно
> берёшь кодекс/сонет как замену, а потом пик проходит — но ты уже и дальше не берёшь глм"

Confirmed from this session's dispatch journal: lane after lane resolved
`arm_resolved job=build arm=sonnet reason=codex_quota_gate_80pct`, for hours, with no attempt to
return to GLM once conditions changed. GLM-FIRST-01 is the standing repo decision — GLM is the
primary code writer and Sonnet is the exception list — so a router that drifts permanently onto
Sonnet has quietly inverted a founder decision without anyone deciding anything.

## Two distinct things to establish, in order

1. **Why is the fallback chosen at all right now?** The resolver's three live readings
   (`live_glm_pct`, `live_codex_weekly_pct`, `live_anthropic_pct`) all return **None** — unknown —
   when called directly. The resolver's own docstring says an unknown read should not block an arm
   ("an unmeasured bucket beats an outage"), and that only a *known* over-threshold reading blocks.
   Yet the emitted reason is `codex_quota_gate_80pct` and the outcome is Sonnet.
   Establish what the resolver actually sees at dispatch time versus what it returns when called
   cold — if it is treating an unknown GLM reading as a reason to skip GLM, that alone is the bug,
   and it inverts GLM-FIRST every time a quota probe is unavailable.
2. **Is there any recovery path?** Even when the fallback is correct at the moment it is taken,
   nothing re-evaluates later. A quota window expires, the peak passes, and the router keeps
   choosing the fallback because nothing ever asks the question again. Find where the decision is
   cached or carried forward, and make each dispatch re-resolve from live conditions rather than
   inherit a stale verdict.

## What to build

Whatever the diagnosis supports — but the end state is: **each dispatch resolves the arm from
current conditions, and an unknown reading never silently demotes GLM.** If the fallback is right
now, it is taken now; when conditions change, the next lane goes back to GLM without anyone
noticing or intervening.

Also make the choice legible: the reason string should say what was actually read
(`glm=unknown codex=91% anthropic=44%`), not just name a rule. Today's `codex_quota_gate_80pct`
tells the lead a rule fired but not what the world looked like — which is why this went unnoticed
for hours.

## Hard constraints
- `~/Projects/leadv2` is the single source — never a real copy inside a consuming project.
- Do not hardcode an arm exclusion or a hand-kept arm list: quota, task kind and complexity decide.
- No deploy. The lead merges. Do not touch `docs/leadv2/open-threads.md`.

## Note on dispatch in this repo
Lanes here previously died at `architect_prepass` rc=124 — diagnosed, not a hang
(`docs/handoff/PLUGIN-PREPASS-HANGS-01/report.md`); raise `LEADV2_DISPATCH_ARCHITECT_TIMEOUT_SEC`
if you hit it.

## Deliverable
The fix plus tests, and `docs/handoff/GLM-FIRST-RECOVERY-01/report.md` — what the resolver was
actually doing, and proof that a lane returns to GLM once conditions allow. End with
DELIVERABLE_COMPLETE.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-24bab870" "<question>" \
