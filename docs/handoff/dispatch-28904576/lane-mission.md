Product implementation task dispatch-28904576. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# GUARD-RESET-FLAG-GAP-01 — deny-floor regex misses flag-interspersed git forms

Repo: ~/Projects/leadv2. Live incident 2026-08-24: `git -C ~/Projects/leadv2 reset --hard HEAD`
passed BOTH the Claude-side deny-floor hook AND the new lv2guard, and executed in the shared
tree (wiped uncommitted files). The plain `git reset --hard HEAD` is correctly refused by both.
Root cause: the `git_reset_hard` regex in plugins/leadv2/config/leadv2-deny-patterns.yaml
matches `git reset --hard` adjacently and is defeated by interspersed global flags (`-C <path>`,
`--git-dir`, `-c key=val`).

## Deliverables
1. Harden the affected regexes in leadv2-deny-patterns.yaml so git global flags between `git`
   and the subcommand do not defeat them. Audit ALL git rules in the file for the same gap
   (reset --hard, clean, stash drop/clear, worktree prune/remove, push --force if present, …) —
   one finding = a pattern census, not a single-rule fix. Keep patterns anchored enough to not
   false-positive on prose or paths containing "git reset".
2. Same census for plugins/leadv2/codex-lead/deny-extra.yaml.
3. Tests: extend the existing deny-floor/lv2guard test suites with flag-interspersed cases
   (`git -C x reset --hard`, `git --git-dir=... clean -fd`, `git -c a=b stash drop`) asserting
   REFUSED, plus a negative case (`echo "git reset --hard"` in prose args) asserting allowed
   where the current suite asserts that shape.
4. bash -n + both suites green.

Constraints: minimal diff; only the two yaml files + tests (+ shared regex helper if one
exists). HARD RULE: no halting on prepass mismatches; proceed and note in summary.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-28904576" "<question>" \
