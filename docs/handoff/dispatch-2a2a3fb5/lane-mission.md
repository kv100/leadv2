Product implementation task dispatch-2a2a3fb5. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# REVIEW-ROUND1-EXHAUSTIVE-01 — round-1 review is exhaustive, round-2 is verification-only

## Outcome (one)
The critic/review mission text the plugin sends to review arms is restructured so that:
- ROUND 1 = exhaustive multi-lens pass: correctness · tests-can-fail (falsification) ·
  product-invariant/contract · census ("if you find one instance of a defect shape, enumerate ALL
  same-shape instances in the touched files before returning") · explicit instruction to report
  EVERYTHING found in one pass, never stop at the first 1-3 findings;
- ROUND 2+ (when the prior round's verdict exists) = verification-only: verify each prior finding
  fixed by execution; new findings admitted ONLY if introduced by the fixes.

## Premise (2026-08-19)
Journal analysis: reviews fail with 1-3 highs per round and tasks burn 3-8 rounds because defects
surface layer-by-layer (recorded failure_class review:stacked-defects-two-rounds). Manually running
this two-phase prompt on REVIEW-GATE-INFRA-01 today gave: round 1 = 12 findings in one pass,
round 2 = pure verification PASS_WITH_NITS. Founder-approved batch item 3.

## Where
Find the review mission/prompt assembly the plugin actually uses on the live path (the text
review arms receive): likely inside plugins/leadv2/scripts/leadv2-review-run.sh (mission heredoc)
and/or plugins/leadv2/skills/leadv2-review/. Confirm the live reader by tracing what
leadv2-review-run.sh sends to arms — do not edit a dead template (verify by grep of the exact
phrases the arms' saved review-mission-*.md files contain, e.g. docs/handoff/dispatch-886a5711/).
Round detection: a prior verdict artifact for the same task (review-gate.md / recorded verdict)
⇒ round 2+; pass the prior findings body into the round-2 mission.

## Write set
plugins/leadv2/scripts/leadv2-review-run.sh (mission text + round detection) ·
plugins/leadv2/skills/leadv2-review/** (if that is the live text source) ·
plugins/leadv2/scripts/tests/** (new suite).

## Non-goals / off_limits
- leadv2-dispatch-product-close.sh (another lane owns it right now — do NOT edit).
- No changes to arm selection/pool/quota logic in review-run — ONLY the mission text assembly
  and round detection.

## Acceptance
- New test: round-1 mission contains the multi-lens + census + report-everything directives;
  with a prior verdict artifact present, the generated mission is verification-only and embeds
  the prior findings; red-first against pinned pre-fix baseline.
- run-core-offline.sh 50/0; bash -n + shellcheck clean.

## Rollback
Single revert.

## Terminal artifact
Commit + the two generated mission texts (round-1 and round-2 samples) + raw test output +
DELIVERABLE_COMPLETE.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-2a2a3fb5" "<question>" \
