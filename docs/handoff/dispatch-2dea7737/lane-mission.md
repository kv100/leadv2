Product implementation task dispatch-2dea7737. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# Plugin follow-ups — round 1b (verify and finish an abandoned round 1)

Repo: `/Users/kostiantyn.vlasenko/Projects/leadv2`. Lane `a24b1588`, HEAD `5e69c0b`.
Do not rebase. **Do not move the branch backwards past any commit** — that happened to another lane
today and orphaned a lead commit.

Your spec is `docs/handoff/plugin-followups-2026-08-06/mission-r1.md` (in the persona-engine repo at
`/Users/kostiantyn.vlasenko/Projects/persona-engine/docs/handoff/plugin-followups-2026-08-06/mission-r1.md`).
Read it in full — all three items, their rationale and their verification bar still bind. This file
only describes the state you inherit.

## What you inherit is CODE WITHOUT EVIDENCE

Round 1's worker died silently (the fourth such death this session — process gone, no sentinel,
every log sink empty). I committed what was on disk as `5e69c0b` so it would not be lost. **I ran
none of it. No suite was executed, no fail-then-pass was demonstrated, no counts were reported.**

On disk, all unverified:
- `plugins/leadv2/scripts/leadv2-dispatch-code.sh` (+76) — presumably item 1: routing router_v2's
  `v2_eligible` arm selection through the same dispatchable filter as v1.
- `plugins/leadv2/scripts/tests/test-router-v2-retired-arm.sh` (new, 199 lines) — presumably the
  `LEADV2_ROUTER_V2=1` test that a retired arm cannot be selected.
- `test-arm-ladder-vocabulary-drift.sh` (+66) — presumably extending drift coverage to the v2 path.
- `test-routing-enforcement-p1.sh` (+25) — presumably item 2, the Test 6 `PROJECT_ROOT`/cwd
  hermeticity fix.
- `test-dispatch-resume-sentinel.sh` (+21) — presumably item 3, S7 asserting `rc == 0`.

Treat every one of those as a draft by someone who never checked their work. If any of it is wrong
or half-done, fix it properly — you are not obliged to keep it.

## Your job

1. **Audit before extending.** Run everything. Report what actually passes and what does not.
2. **Produce the evidence round 1 never produced**, per mission-r1: each fix needs a test that FAILS
   against the plugin repo's `main` (currently `a1afed9`) and passes at your HEAD — both runs shown
   verbatim, proven with a scratch copy or temp worktree, never `git stash`/`reset --hard`/`clean`.
3. **Finish anything incomplete**, especially the item-1 requirements: the retirement must stay
   expressed as config both v1 and v2 honour (never a second hand-kept exclusion list), the drift
   suite must fail when v1 and v2 disagree, and **no test may spawn a real provider session** — grep
   for any path reaching a real `kimi-coder.sh` / `glm-coder.sh` / `codex` and prove it cannot.
4. **Item 2 needs the cross-cwd demonstration** mission-r1 asked for: same result from three
   different cwds, one of them inside a repo that HAS a routing config.
5. **Report the FULL suite counts**, not just the files you touched — that omission is exactly how a
   pre-existing failure stayed invisible in an earlier close. Name which failures are pre-existing
   and verify that against `a1afed9` yourself rather than repeating my claim. Do not fix
   `test-lane-liveness-authoritative.sh`'s C2 here; just make it visible and say whether it warrants
   its own task.

## Guard against your own death
Commit incrementally — after each item, not once at the end. Four workers have died mid-task today
with no trace in any log; an incremental trail means the next one resumes instead of restarting.

## Return
`PASS|FAIL|BLOCKED` + commit sha + the failing-then-passing run of every new/changed test + full
suite counts with pre-existing failures named + the cross-cwd demonstration + your answer on the C2
failure + an explicit statement of what in `5e69c0b` you kept, changed or discarded, and why.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-2dea7737" "<question>" \
