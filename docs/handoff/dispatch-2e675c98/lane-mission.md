Product implementation task dispatch-2e675c98. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CLAIM-EVIDENCE-GATE-01 — RESUME (lane d784b987, prior worker stalled mid-work)

The lane worktree already contains the in-progress edits (uncommitted):
- plugins/leadv2/skills/leadv2-subagent-protocol/SKILL.md — UNVERIFIED-contract text (verify it
  matches the mission contract below; keep/fix)
- plugins/leadv2/scripts/leadv2-review-run.sh — fifth review lens (verify placement is in the
  round-1 exhaustive mission text only; no logic changes)
- plugins/leadv2/scripts/claude-subsession.sh — inspect why it was touched; keep only if it is
  the live preamble injector for the subagent protocol (cite the reader line), else revert it.
Start by reading `git -C . status --short` and `git diff` in the worktree — finish, don't redo.

## Original contract (unchanged)
1. Subagent deliverable contract: every factual claim about an external system/API must carry an
   inline probe artifact OR an explicit `UNVERIFIED:` tag; untagged evidence-free claim = protocol
   violation. Must live in the LIVE preamble source (prove the reader).
2. Review lens in round-1 exhaustive mission text (leadv2-review-run.sh): "claims-without-evidence"
   — untagged unverified claim that drives a decision = BLOCKING; tagged = MEDIUM at most.

## Remaining work
- Tests: red-first against pinned pre-fix baseline (content-probe pattern per
  test-review-gate-scope-evidence.sh; GREEN_PRE_FIX>0 → non-zero exit). Extend
  test-review-round-exhaustive.sh or add a suite; register any new suite in run-core-offline.sh
  SUITE_DEFS. Suites must be hermetic: never mutate tracked files of the real repo and restore any
  fixture they touch (this exact defect recurred today).
- run-core-offline.sh green forward AND LEADV2_CORE_OFFLINE_REVERSE=1; bash -n + shellcheck on
  changed scripts.
- Revert the unrelated docs/handoff/dispatch-*sig*/phases.d/*.yaml churn in the worktree before
  committing (fixture side-effects of earlier test runs, not deliverable).
- COMMIT on the lane branch.

## Off_limits
leadv2-dispatch-product-close.sh; anything outside the write set above.

## Terminal artifact
Commit sha + both text samples (lens + contract) + live-reader proof + raw red/green + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-2e675c98" "<question>" \
