Product implementation task dispatch-34721538. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# SUPERVISOR-DELETE-01 — ROUND 2 FINISHER (resume of lane eacd0eb5)

Round-1 worker did phase-1 (migration) and died before finishing phase-2 (deletion), leaving 47
uncommitted files. Current worktree state (verified by the lead):
- DONE: leadv2-supervise.sh renamed → leadv2-lanes-snapshot.sh (live-path reconciliation kept),
  callers updated (task-anchor/pre-compact hooks, plugin-sync, status-collector, 7 suites).
- NOT DONE: leadv2-supervise-{loop,pick,resume,watchdog}.sh still exist; supervise test suites
  still exist and 2 are red against the renamed subject; SUITE_DEFS still registers
  "supervisor fail-closed" (test-supervise-failclosed.sh), "supervisor reconciliation"
  (test-supervise-v2.sh), "supervisor/lead PID isolation" (tests/test-supervise-fanout-guard.sh).

Start with `git status --short` + `git diff --stat` in the worktree — FINISH, do not redo.

## Remaining work (phase 2)
1. git rm: leadv2-supervise-loop.sh, leadv2-supervise-pick.sh, leadv2-supervise-resume.sh,
   leadv2-supervise-watchdog.sh. For each, FIRST grep the repo for callers; any live-path caller
   gets migrated or the call removed (cite the grep).
2. Test suites: delete test-supervise-v2.sh, test-supervise-failclosed.sh,
   tests/test-supervise-fanout-guard.sh AND their 3 SUITE_DEFS rows. For the other supervisor-named
   suites (test-statusline-supervisor-gate, test-supervisor-close-plan-mirror,
   test-supervisor-mode-reinject, test-supervisor-reason-honest): decide per suite — if the subject
   it tests survives (e.g. the mode-reinject hook still exists), KEEP and rename/retarget if
   needed; if the subject died with the loop, delete it and its SUITE_DEFS row. Cite the subject
   check for each.
3. If lanes-snapshot (renamed reconciliation) still deserves a suite, retarget the useful parts of
   test-supervise-v2.sh into test-lanes-snapshot.sh (adoption/prune/tombstone cases) and register
   it — do NOT lose the coverage silently; the symlinked fake-claude trick (see the 512ecda hunk)
   stays.
4. docs: plugins/leadv2/docs/supervisor-role.md → delete or replace with a 3-line "retired
   2026-08-19 (founder order), reconciliation lives in leadv2-lanes-snapshot.sh" stub; /leadv2
   supervise skill entry → refuse-stub.
5. Acceptance: run-core-offline.sh forward green in the lane (row count reflects removals);
   bash -n + shellcheck -S warning on every EDITED script; grep proof: zero references to the four
   deleted -loop/-pick/-resume/-watchdog scripts anywhere in plugins/leadv2 outside docs archive.
6. COMMIT everything on the lane branch (migration + deletion may be one or two commits).

## Off_limits
leadv2-dispatch-product-close.sh (T2 lane owns it, LIVE right now — do not touch);
leadv2-dispatch-code.sh routing block (V3-GLM lane owns it, LIVE — only touch if a supervise
caller must be removed, and then ONLY that call site).

## Terminal artifact
Commit sha(s) + per-item classification + raw greps + suite output + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-34721538" "<question>" \
