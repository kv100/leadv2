Product implementation task dispatch-456a9264. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# REVIEW-ROUNDCAP-01 — fix round 1 (critic FAIL, narrow finisher)

Worktree exists: /Users/kostiantyn.vlasenko/Projects/leadv2/.claude/worktrees/e8c1289f
(branch worktree-e8c1289f). Work ONLY there. Four fixes, no refactors.

1. [HIGH] leadv2-review-run.sh `_review_state_write` (~:767-796) + `_review_roundcap_read`
   (~:693-712), used at ~:993-1024 and ~:1109-1141: the attempts/spawns read→increment→mv has
   NO cross-process lock — two concurrent engine runs can both read N and both write N+1,
   losing an increment (cap under-fires). Wrap the read+increment+write critical section in the
   SAME `lv2_lock_wait` primitive the sibling diff-hash ledger uses (see
   leadv2-dispatch-code.sh:2556-2579 `atomic_review_check_and_record` for the exact pattern,
   lock file beside the state file, 10s wait, fail-open on lock timeout → run the review).
2. [HIGH] leadv2-review-run.sh ~:1006 and the spawncap mirror ~:1122: the `escalation:` field
   written into review-gate.md hardcodes `docs/handoff/dispatch-${TASK}/review-roundcap-escalation.md`
   while the actual file is written to `${HANDOFF}/review-roundcap-escalation.md`. Under a
   non-canonical --handoff the pointer names a nonexistent path. Fix both sites to use
   `${HANDOFF}/review-roundcap-escalation.md`. Do NOT touch the pre-existing same-shaped
   convention at ~:1467/:1480 (out of scope, pre-existing).
3. [MED] leadv2-dispatch-product-close.sh ~:2237-2250: `case ${_engine_rc}` lacks an `8)` arm —
   rc=8 (roundcap/spawncap refusal) falls to `*` and journals `review_engine_error`. Add an
   explicit `8)` arm journaling `review_roundcap` (mirror the wording of the existing arms).
4. [LOW] leadv2-review-run.sh spawncap block: `${_review_roundcap_max:-$(_review_roundcap_limit)}`
   — the fallback is dead (var always set). Simplify to the var.

Also: add ONE test to test-review-roundcap.sh that exercises the lock path (two sequential
invocations sharing state is already covered; for concurrency, at minimum assert the lock file
is taken during increment — e.g. hold the lock externally and verify the engine either waits or
fails open per the chosen semantics; keep it deterministic, no sleep-race tests).

Then: bash -n on touched files; run test-review-roundcap.sh (all pass) and
test-review-round-exhaustive.sh (24/24, no regression); commit in the worktree naming e8c1289f.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-456a9264" "<question>" \
