Product implementation task dispatch-4a611b75. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CLAIM-EVIDENCE-GATE-01 — ROUND 2 FINISHER (lane d784b987, on top of commit 98ce586)

Round-1 verdict FAIL (diff=653b7648, reviewer=opus-critic, full report:
~/Projects/leadv2/docs/handoff/dispatch-2e675c98-review/critic.full.md — READ IT FIRST).
Fix H1-H2, M1-M5, L1-L5 (L6/L7 informational). Findings verbatim:

H1 claude-subsession.sh:285 + SKILL.md:186-196 — `$PROJECT_ROOT/.claude/skills/leadv2-subagent-protocol/SKILL.md`
   does not exist in persona-engine / m3-market / respiro-ios; awk fails open to empty, so §11
   never enters a worker prompt (rendered-prefix probe showed 0 matches). Fix: resolve skill_file
   from the PLUGIN root (with repo-local override allowed) + warn-don't-silence on empty; keep the
   SKILL.md hunk only if it is actually rendered.
H2 leadv2-review-run.sh:705 vs leadv2-dispatch-code.sh:2532,2696 / glm-coder.sh:229 — GLM/Kimi/
   Codex arms never receive the contract text, yet round-1 BLOCKS untagged claims (rule with no
   reader on the writing side). Fix: inject the evidence-contract into all four arms' mission
   assembly (dispatch-code + glm-coder paths), or (fallback) downgrade the lens to MEDIUM until
   covered — prefer the injection.
M1 test-claim-evidence-gate.sh:65-70 — C1 is a bare source grep of the edited file; add a rendered-
   artifact probe (LEADV2_DRY_RUN=1 prefix-file assertion via run_case) like C6. This is exactly
   why H1 shipped green.
M2 test-claim-evidence-gate.sh:198 — `printf … "${ERRORS[@]}"` breaks under /bin/bash 3.2 set -u
   when empty; use "${ERRORS[@]+"${ERRORS[@]}"}" and push green-pre-fix names into ERRORS.
M3 test-supervise-v2.sh hunk is a byte-identical duplicate of 512ecda (already on origin/main):
   rebase the lane onto origin/main and DROP the hunk.
M4 test-supervise-v2.sh:267-271 comment — two untagged macOS claims driving the cp→ln change
   (both true, reviewer probed rc=137 / comm=./…): inline the probe artifacts into the comment or
   tag UNVERIFIED:. (After M3's rebase this lands wherever the comment lives on main — if the
   comment is main's, leave main alone and skip M4; do not touch main from this lane.)
M5 claude-subsession.sh:250-252 — says BLOCKING unconditionally; enforcement lives only in the
   round-1 branch. Add the round-1 qualifier (SKILL.md:192-194 wording is correct — mirror it).
L1 dangling "See full protocol:" pointer (claude-subsession.sh:253) — point it at the real path.
L2 test :56 fallback HEAD guarantees red without origin/main — pin the SHA like sibling tests.
L3 C4 quote/backtick guard misses line 704 flowing into --focus — extend the guard.
L4 self-nullification guard checks only leadv2-review-run.sh — extend to the other edited files.
L5 PREFIX_DIR leaks on FATAL/interrupt — add a trap.

## Acceptance
- Rendered-prefix probe GREEN in a project repo context (simulate persona-engine root): the
  evidence-contract text appears in the assembled worker prompt for claude arm AND for
  glm/codex mission text (raw grep output in the terminal artifact).
- test-claim-evidence-gate.sh red-first still real (baseline 559cf15), now with the rendered-
  artifact case; suite green under bash 3.2 AND 5.x.
- run-core-offline.sh forward + LEADV2_CORE_OFFLINE_REVERSE=1 green in the lane.
- bash -n + shellcheck -S warning on changed files.
- COMMIT on lane branch (rebase onto origin/main first per M3).

## Off_limits
leadv2-dispatch-product-close.sh; no new env vars; no round-detection/arm-selection logic changes.

## Terminal artifact
Commit sha + per-finding note + raw probe outputs + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-4a611b75" "<question>" \
