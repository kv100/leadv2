Product implementation task dispatch-4b7593fe. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# MISSION — REPORT-ONLY-GATE-01, round 2 (rebase onto current plugin main; one conflict)

Plugin repo `~/Projects/leadv2`, same worktree. **The work is correct and reviewed — 8/8 plus 5/5
red-first against pre-fix. Do not redo it.** This round is only about landing it.

## What happened

The lane branched from `b90e40e`, and plugin `main` has since advanced to `bb400e9` (it now carries
`GLM-FIRST-RECOVERY-01`). The merge conflicts in one file:

`plugins/leadv2/scripts/leadv2-dispatch-product-close.sh` — main gained
`43ecb8c feat(review-gate): REVIEW-GATE-SHOWS-FINDINGS-01` and
`105e07f fix(reliability): PR02 surgical items 1-4` in the same region this lane edits.

## What to do

Rebase onto current `main` and resolve, keeping **both** behaviours:

- main's: the review gate prints findings rather than bare counts, and the PR02 reliability fixes
  (run_dir at both reap call sites, group signalling for pgid, grace-branch reorder).
- this lane's: a report-only lane is gated on its deliverable file, harvested to
  `docs/handoff/dispatch-<TASK>/report.md`, and still prose-reviewed.

These are complementary, not competing — one decides *what* the gate judges, the other decides *how*
it reports. If the resolution makes them look mutually exclusive, that is a sign the merge is wrong,
not that a choice is needed.

Re-run both suites after the rebase — the conflict is in the file the tests exercise, so a green
result from before the rebase proves nothing now.

## Hard constraints
- `~/Projects/leadv2` is the single source. No deploy. Do not touch `docs/leadv2/open-threads.md`.

## Deliverable
The rebased branch, both suites green and pasted. End with DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-4b7593fe" "<question>" \
