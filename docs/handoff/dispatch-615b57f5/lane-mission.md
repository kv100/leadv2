Product implementation task dispatch-615b57f5. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BURN-GOVERNOR-01 — fix round 1 (critic findings, narrow finisher)

Worktree already exists: /Users/kostiantyn.vlasenko/Projects/leadv2/.claude/worktrees/b22dc98b
(branch worktree-b22dc98b). Work ONLY there. Three fixes, nothing else — do not refactor.

1. [HIGH] The burn gate defaults ON and reads the REAL ~/.claude/burn/history.db, so any
   pre-existing test that drives leadv2-dispatch-code.sh through a real resolve now fails on a
   host whose live 24h burn exceeds the hard cap (reproduced: test-glm-deferred-ladder.sh
   subtest (f) gets rc=6). Fix: add `export LEADV2_BURN_GOVERNOR=0` to the ambient-env scrub
   block of every test under plugins/leadv2/scripts/tests/ (and tests/ at repo root if they
   exercise dispatch-code resolution) that invokes leadv2-dispatch-code.sh — audit with
   `grep -rl "leadv2-dispatch-code" <both test dirs>` and check each hit's env setup. The new
   test-burn-governor.sh must keep its own explicit on/off control (don't blanket-disable
   inside it). Re-run test-glm-deferred-ladder.sh after: subtest (f) must no longer fail with
   rc=6 / BURN GATE (its other pre-existing (d)/(e) failures are known-on-main; leave them).
2. [MED] hooks/leadv2-compact-trigger.sh (~line 328 emergency MSG): the emergency-level warn
   text must state plainly the session is past emergency context width and name
   LEADV2_COMPACT_INTERACTIVE_BLOCK=1 as the opt-in for auto-/compact (default OFF per founder
   order 2026-08-19, commit 7daf57f — do NOT change the default, it is already correct).
3. [MED] scripts/leadv2-burn-governor.sh (~line 114): the sqlite3 call has only
   `-cmd '.timeout 2000'` (covers SQLITE_BUSY only). Wrap the invocation in a hard wall-clock
   bound (`timeout 5 sqlite3 …` if GNU timeout exists, else a portable perl/python alarm or
   background+wait fallback — this repo runs on macOS, `timeout` may be absent; check how
   sibling scripts bound wall-clock and copy that pattern). On timeout → verdict=ok
   reason=no_telemetry (fail-open), exit 0.

Then: bash -n on every touched file; run test-burn-governor.sh (must stay 25/25) and
test-glm-deferred-ladder.sh (subtest f clean); commit in the worktree naming b22dc98b.
Findings 4/5 from review (retry-all TOCTOU, founder_task_id drop) are inherited from
cmd_glm_deferred and explicitly OUT OF SCOPE — do not touch.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-615b57f5" "<question>" \
