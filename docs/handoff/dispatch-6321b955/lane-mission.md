Product implementation task dispatch-6321b955. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# LANE-REGISTRY-SELF-DEADLOCK-01 — fix round (e2e_regression)

Repo: ~/Projects/leadv2, lane worktree .claude/worktrees/8e705910 (continue on the existing
branch, wip commit f933fb5 is yours). Original mission and your changes stand; e2e gate failed
on ONE suite:

FAIL: tests/run-core-offline.sh → "fanout classifier/runner guard", Test 4:
  leadv2-fanout.sh:52 sources
  <fixture-home>/.claude/leadv2-shared/scripts/leadv2-active-registry.sh — No such file or
  directory. The fixture home the suite builds does not contain the helper your change makes
  fanout load (or your change resolves the helper via $HOME instead of the repo/canonical
  fallback chain).

Fix so the suite is green: prefer resolving the helper via the existing canonical-fallback
idiom (SCRIPT_DIR first, then LEADV2_CANONICAL_ROOT, $HOME-shared LAST), and/or stage the
helper into the fixture home in the test setup — match how the other core-offline fixtures
stage shared scripts. Do not weaken the assertion.

Deliverables: green run of tests/run-core-offline.sh (paste tail in summary), bash -n, commit
on the lane branch. HARD RULE: no halting on prepass mismatches; proceed.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-6321b955" "<question>" \
