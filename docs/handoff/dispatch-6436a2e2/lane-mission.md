...
## Addendum 4 — the acceptance is DONE except for the one thing that makes it durable

Commit `f495f881` on `worktree-DOD-GATE-CHARGES-LANES-FOR-HARNESS-WRITES-01` finished the census,
the tracked-copy decision on both mains, all four acceptance items and a 199-line report. It will
not be merged, and the reason is exact: **`git diff --name-only main...HEAD | grep -c test` is 0.**
Seventeen files, not one of them a test.

So the negative control — `NEG: dod_fail check=runtime_state_in_diff paths=docs/LEAD_V2_STATE.md
rc=1` — was a **hand demonstration**. It proved the mechanism at that moment and cannot survive the
next change, because CI will never select or execute it. By our own standing rule a green suite CI
never runs is worth nothing, and a suite that does not exist is worth less.

**Your whole job this round is to make that one demonstration permanent.** Do not redo the
relocation, the census, or the report — review them and leave them alone.

1. A suite that pins exactly what was shown by hand: restore the registry write into a lane
   worktree, assert the gate goes red with `dod_fail check=runtime_state_in_diff`; remove it,
   assert green. The red must come from the production check, not from a stub of it.
2. A `# run-all-triggers:` header naming the files whose change must select this suite — at minimum
   `leadv2-active-registry.sh` and the state-path resolver.
3. Proof of selection: run `--scope changed` against your own diff and show the suite in the
   selected list. A suite CI does not select is the failure this task exists to prevent.

## Addendum 5 — why your last close said `no_work`, and why the e2e budget is raised

Two instrument defects, both filed, neither yours to fix here:

- `CLOSE-GATE-CALLS-A-FINISHED-LANE-no_work-01` (P0): your previous round was stamped
  `terminal=no_work cause=empty_diff` while the branch carried 3 commits and 363 insertions. A
  population census by the other lead found **9 of 14 checkable `no_work` verdicts were false**.
  If your close says `no_work`, check `git diff --stat main...HEAD` before believing it.
- `E2E-GATE-CANNOT-PASS-BY-CONSTRUCTION-01` (P0): `tests/run-all.sh:114-118` adds
  `run-core-offline.sh` on every scope and core-offline alone exceeds 10 minutes, so the 900 s
  budget cannot be met by any change. This lane runs with `LEADV2_PHASE8_E2E_TIMEOUT_S=2700` — a
  named suppression so you can reach a close, not a fix. Do not touch the default.
