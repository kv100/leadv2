Product implementation task dispatch-6a5cd248. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CTX-COST-GUARDS-01 (v3) — two plugin fixes: the self-inflicted scope refusal, then the diff guard

Work ONLY in `~/Projects/leadv2` (`plugins/leadv2/`), inside the lane worktree you are pinned to.

## FIX A (do this first) — the gate refuses a lane for dirt the plugin itself created

**Proven, do not re-derive.** Two consecutive lanes died at
`leadv2-dispatch-product-close.sh` with `status: blocked, reason: unscoped_lane_work`, and the
gate's own evidence names the offender:

```
dirty: 1
offending: .claude/commands/leadv2.md
```

That path is a **symlink the plugin's own bootstrap creates in every fresh worktree**
(`.claude/commands/leadv2.md -> …/plugins/leadv2/commands/leadv2.md`, ctime = worktree creation).
No worker wrote it. But `_PC_PORCELAIN_EXCLUDE_RE` (line ~1192) only excludes `docs/leadv2/`,
`docs/handoff/`, `docs/LEAD_V2_STATE.md`, `__pycache__/`, `*.pyc` — so the partition at ~2107
counts it as ONE undeclared dirty path and refuses. **The plugin dirties its lane and then fails
the lane for being dirty.**

Fix it at the source:

1. Extend `_PC_PORCELAIN_EXCLUDE_RE` to cover plugin-owned bootstrap dirt in a lane worktree.
   Derive the list from what `scripts/leadv2-repo-install.sh` actually creates (read it — do not
   guess): at minimum `.claude/commands/`, and whatever else it symlinks or scaffolds
   (`.claude/scripts/`, `.claude/agents/`, control-plane state dir).
2. **Stay strict.** Exclude such a path ONLY when it is a symlink or an untracked bootstrap
   artifact — a REAL file a worker wrote under the same prefix must still count as a violation.
   State in a comment which predicate enforces that.
3. The exclude constant is used by both `_pc_lane_dirty` and the partition; keep them in sync — the
   existing comment says they must not diverge.
4. When the ONLY dirt is excluded bootstrap, the honest verdict for an empty lane is `no_work`
   with its real cause, never `unscoped_lane_work`.

## FIX B — nothing guards diff reading on the Bash side

`leadv2-read-gate.sh` (PreToolUse:Read, registered) already merges force-read-limit +
read-dedup-hard + lead-read-guard. `leadv2-bash-output-cap.sh` is PostToolUse — it warns after the
tokens are paid. `leadv2-bash-pre-dispatch.sh` (PreToolUse:Bash) has exactly two sub-guards
(`leadv2-block-bash-heredoc`, `leadv2-block-fg-dispatch`) and nothing about diffs. Measured: the
heaviest Bash calls in the whole transcript history are `sed -n '1,340p' …/review.diff` (7,393
tok), `sed -n '1,220p' …/build-attempt-1.diff` (7,407), one diff read TWICE at 7,383 each.

Add sub-guard `leadv2-warn-bash-diff-read` to `hooks/leadv2-bash-pre-dispatch.sh`, same shape as
the two existing ones:

- Fires when the command pulls a diff into context: a path matching `\.diff$` / `\.patch$`, or
  `git diff` / `git show` without `--stat` / `--name-only` / `--numstat` — AND the read is
  unbounded (`cat`, `less`, `awk`, `grep` with no cap) or its window exceeds
  `LEADV2_DIFF_READ_MAX_LINES` (default 200) for `sed -n 'A,Bp'` / `head -N` / `tail -N`.
- **Advisory WARN, exit 0 by default.** `LEADV2_DIFF_READ_DENY=1` → block (exit 2);
  `LEADV2_DIFF_READ_GUARD=0` → off. A block by default would hit reviewer workers doing their job.
- Skip when the caller is a subagent/worker — reuse the `agent_type` predicate from
  `hooks/leadv2-lead-read-guard.sh`.
- Message: 3 short lines — `git diff --stat` → the handoff dir's `review-findings.json` →
  `repowise distill <cmd>` (`repowise expand <ref>` recovers what was elided).
- Must NOT fire on `--stat` / `--name-only` / `--numstat`, `wc -l`, `grep -c`, `ls`.

## Hard requirements (both)

Fail-open on every error path (bad JSON, missing `jq`, unreadable stdin → exit 0). No network. Do
not touch `hooks.json`. Do not change the two existing sub-guards' behaviour.

## Verification you MUST run and paste

- **Fix A:** construct a scratch worktree containing ONLY `.claude/commands/leadv2.md` as a
  symlink, run the scope partition against it, and show it no longer yields `unscoped_lane_work`;
  then add a real undeclared file and show it STILL refuses. Both outputs, verbatim.
- **Fix B:** red/green table piping real PreToolUse JSON — `sed -n '1,340p' x/review.diff` WARNS,
  `cat /tmp/r4-code.diff` WARNS, `git diff` WARNS, `git diff --stat` silent, `sed -n '1,40p'`
  silent, `wc -l x.diff` silent, subagent `agent_type` silent, `echo 'not json'` → `rc=0`.
- `bash -n` on both touched files, plus one existing heredoc-guard case to prove no regression.

Deliverable: `docs/handoff/CTX-COST-GUARDS-01/developer.full.md` — diffs, both verification
blocks, and one line on anything already covered. Last line exactly `DELIVERABLE_COMPLETE`.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-6a5cd248" "<question>" \
