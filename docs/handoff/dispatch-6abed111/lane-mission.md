Product implementation task dispatch-6abed111. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BROAD-STATUS-RELAY-SCOPE-01 — ROUND 3 FINISHER (resume of lane 91f975bf) — 2 surgical items

Round-2 verification: HIGH-1/HIGH-3 and all MEDIUMs fixed and verified by execution. TWO items
remain (round-2 verdict FAIL, diff=0998e8f1):

1. **Session-id env var is wrong → the whole feature is inert in prod.**
   plugins/leadv2/scripts/leadv2-dispatch-code.sh:405 reads `${CLAUDE_SESSION_ID:-}` — that var
   does NOT exist in a Claude Code Bash subprocess; the real one is `CLAUDE_CODE_SESSION_ID`
   (36-char id, matches ~/.claude/projects/*/<sid>.jsonl). Result today: LEAD_SESSION= empty →
   leadv2_session_has_live_lane never matches → .pulse-beat-owner never written → every session
   unresolved → full 25-row relay everywhere (the original incident, unfixed).
   Fix: read CLAUDE_CODE_SESSION_ID with CLAUDE_SESSION_ID as fallback; ADD A TEST that asserts
   the recorded field is non-empty when only CLAUDE_CODE_SESSION_ID is set (red-first: the test
   must FAIL on the current code).
2. **Round-2 deliverable + ship instruction.** Write the missing deliverable
   (docs/handoff/dispatch-cbe25ccc/developer.full.md or your lane's handoff): name ALL THREE
   cache-sensitive changed hooks (leadv2-single-lead-beat.sh, leadv2-task-anchor.sh,
   leadv2-supervisor-mode-reinject.sh) with the cache-copy + session-restart ship step; remove
   the stale "deliberately not registered"/find-on-every-fire claims.
   Housekeeping: keep untracked founder-status.md/status-snapshot.json and the 2 unrelated
   journal.md edits OUT of the commit.

Then COMMIT the full lane work (r1+r2+r3) on the lane branch.

## Off_limits
Everything else. No new scope. Write-set as before + the one line in leadv2-dispatch-code.sh:405
(session-id read only — do not touch routing/dedup logic).

## Acceptance
- New env-var test red-first (raw both runs); suite green (22+ cases); probe with only
  CLAUDE_CODE_SESSION_ID set shows .pulse-beat-owner written for a live-lane session.
- bash -n + shellcheck; commit on lane branch; deliverable present with 3-hook ship note.

## Terminal artifact
Commit sha + raw red/green + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-6abed111" "<question>" \
