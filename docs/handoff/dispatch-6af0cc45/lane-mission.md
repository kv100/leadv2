Product implementation task dispatch-6af0cc45. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# MISSION — REVIEW-GATE-SHOWS-FINDINGS-01 (the gate hides the findings it was paid for)

Repo: `~/Projects/leadv2` (the plugin source — the founder authorised shared-tree edits on
2026-08-14). Owner file: `plugins/leadv2/scripts/leadv2-review-run.sh`.

## The defect

`review-gate.md` is what the lead reads to decide whether to merge. Today it contained only:
...
Not one word of what the findings were. The actual findings — including two HIGH ones that
invalidated the lane's entire measurement approach — were sitting in `review-<arm>.md`, a file
the lead has no reason to open once the gate has already printed a verdict.

The `pass` case is worse than the `fail` case. A `PASS_WITH_NITS` verdict carrying nine findings
(six of them medium) renders as a bare `status: pass`. **Earlier today the lead merged three
lanes believing the reviewer had no remarks.** A gate that hides remarks behind a green word is
not a gate; it turns a paid review into a rubber stamp.

## What to change

`review-gate.md` must carry the findings themselves, not just their counts. At minimum, for every
finding at medium severity or above: its severity, its one-line claim, and its `file:line`. Keep
the machine-readable header (`status:`, the counts) exactly as it is — other tooling parses it,
and this must stay backward-compatible.

Requirements:
- **A `pass` / `PASS_WITH_NITS` verdict with findings must display them.** This is the case that
  actually bit us. A reader who stops at the first line must still see that remarks exist.
- **Never truncate silently.** If output is capped, say how many findings were omitted and where
  the full text lives.
- **A lost or empty review body must not render as a clean pass.** The existing
  `REVIEW-BODY-PERSIST-01` guard covers part of this; make sure the new rendering cannot
  reintroduce the hole (zero findings parsed out of a non-empty body should read as a parse
  failure, not as "no findings").
- Multi-arm fan-out: attribute each finding to the arm that raised it, and never let one arm's
  silence mask another's findings.

## Constraints
- This is the shared plugin tree: the change lands in **all** consuming repos at once. Keep it
  strictly additive to the file's existing contract.
- Do not change any verdict logic — what passes and what fails is out of scope. This lane changes
  only what the lead is shown.
- Do not touch the write-race note at the top of the file, or the tmp + `mv -f` atomic write
  pattern.

## Evidence required
Render `review-gate.md` from a fixture for each of: `fail` with findings, `PASS_WITH_NITS` with
findings, clean `pass` with none, and a lost body. Paste all four outputs. A claim of correctness
without the rendered output will be rejected.

Report to `docs/missions/REVIEW-GATE-SHOWS-FINDINGS-01-report.md` in this repo. End with
DELIVERABLE_COMPLETE. Do not commit — the lead reviews and commits inside `~/Projects/leadv2`.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-6af0cc45" "<question>" \
