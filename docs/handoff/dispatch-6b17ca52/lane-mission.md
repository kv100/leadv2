Product implementation task dispatch-6b17ca52. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# HOOK-INJECT-DEDUP-01 — fix round 1 (critic BLOCK, narrow finisher)

Worktree exists: /Users/kostiantyn.vlasenko/Projects/leadv2/.claude/worktrees/7bdb16ee
(branch worktree-7bdb16ee). Work ONLY there — the sibling script named in finding 1 lives in
ANOTHER repo (persona-engine) and is OUT OF SCOPE; solve inside the plugin.

1. [HIGH] The urgency exception is decorative: the hash keys on body+today, but the body's
   scheduled-decisions line comes from persona-engine's `.claude/hooks/scheduled-decisions-nearest.sh`,
   which has its own id-only per-session suppression (`nearest_id == prev_id → exit 0`,
   classification-blind). If the same nearest row flips DUE→OVERDUE, the renderer emits nothing,
   so the new gate never sees the change. FIX INSIDE THE PLUGIN (critic's option B): in
   `plugins/leadv2/hooks/leadv2-task-anchor.sh`, compute the DUE/OVERDUE classification
   INDEPENDENTLY inside the gate — parse `docs/leadv2/scheduled-decisions.md` directly for the
   nearest actionable row's (id, due-date) and derive status=DUE_TODAY/OVERDUE/FUTURE from
   today's date; mix `id:status` into the hash input (alongside body+today). Reuse/borrow the
   date-parse approach the sibling uses (read it for reference only — do not edit it). When the
   classification flips, the hash changes → full re-inject even though the rendered body is
   byte-identical. Keep it cheap (grep+date math, no python startup if the file is absent).
2. [MED] test-inject-dedup.sh: add a REAL scheduled-decisions fixture — create a
   `docs/leadv2/scheduled-decisions.md` in the test sandbox with one row whose Due date the test
   controls; assert: (a) same-day second fire → dedup marker; (b) flip the row to OVERDUE (or
   move the date across the boundary) with body otherwise unchanged → full re-inject.
3. [MED] test-inject-dedup.sh R2 block: also assert the pre-existing
   `/tmp/.leadv2-task-anchor-full-<sid>-*` glob is cleared by the PreCompact branch added in
   `leadv2-pre-compact-checkpoint.sh` (the branch exists — cover it).
4. [LOW] docs/context-diet.md §inject-dedup: correct the overclaim — note the classification is
   computed in-gate precisely because the upstream renderer's own suppression is id-keyed.
5. [LOW] The two broad `except Exception` fail-open blocks in leadv2-task-anchor.sh: add a
   single stderr WARN line (`[inject-dedup] fail-open: <err>`) so a future bug doesn't hide as
   silent always-full-inject. Keep exit 0 semantics.

Then: bash -n; run test-inject-dedup.sh (all pass incl. the new fixture tests); commit in the
worktree naming 7bdb16ee.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-6b17ca52" "<question>" \
