Product implementation task dispatch-6cf5d07e. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BUILDER-SELFCHECK-GATE-01 — ROUND 3 FINISHER (resume lane 9c027877)

The r2 codex worker died mid-run (its later respawns died in <3s). Its work is ON DISK,
uncommitted: lib/leadv2-builder-selfcheck.sh (exists, 14.4KB), gate wired into
leadv2-dispatch-product-close.sh (+41), mission paragraph in leadv2-dispatch-code.sh (+17),
suite row in run-core-offline.sh (+1), tests/test-builder-selfcheck-gate.sh present.
FINISH — do not redo. Start with `git status --short` + `git diff` in the worktree.

The binding spec is the ROUND-2 mission findings (C1-C3, H1-H3, M1-M4) at
/private/tmp/claude-503/-Users-kostiantyn-vlasenko-Projects-persona-engine/cd6c46e0-7993-40b2-9ee9-9727c4eb431b/scratchpad/mission-builder-selfcheck-round2.md
— READ IT FIRST. Your job:
1. Audit the on-disk state against each r2 finding; complete any finding not yet addressed.
   Per finding: cite file:line of the fix or implement it.
2. Run the full r2 Acceptance set and paste raw outputs:
   - recursion probe: selfcheck inside a leadv2-repo lane completes WITHOUT recursive
     product-close (show the depth-guard journal line);
   - the 3 diff-caused suites (review body persist / product-close waits-for-worker /
     report-only gate C4) green WITH LEADV2_BUILDER_SELFCHECK=1;
   - gate-ordering probe (bash -n fail → selfcheck_failed, no review arm spent);
   - test-builder-selfcheck-gate.sh red-first legs + green run;
   - run-core-offline.sh forward green in the lane; delta vs main = only your suite row.
3. bash -n + shellcheck -S warning on every changed shell file.
4. COMMIT everything on the lane branch (single commit ok).

## Lead facts (08-20 07:0x, after two worker deaths)
- Prior attempts checkpointed: f2cc0bc (r2 work) + 7ffe67c (merge main) are committed on the
  lane; only lib/leadv2-builder-selfcheck.sh has a 1-line uncommitted edit. Audit + finish.
- Do NOT launch run-core-offline/run-all in the background and idle-wait on it — that is how
  the last two workers died of worker_timeout. Run suites FOREGROUND and continue when they
  return. Budget: full core-offline takes ~10-15 min solo.
- Known main flakes NOT yours: "dispatch refusal fallback chain" (routing-enforcement-p1) and
  "plan-followups-01" are red on canonical main under the runner — do not count them against
  your lane; your delta = only your suite row.

## Off_limits
leadv2-review-run.sh; anything outside the r1/r2 write-set. The dispatch-code.sh routing
block (candidate_arms/quota) belongs to another live lane — your +17 mission-paragraph hunk
is fine, do not touch routing.

## Terminal artifact
Commit sha + per-finding classification (done-by-r2 / fixed-by-you) + raw probe outputs +
DELIVERABLE_COMPLETE.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-6cf5d07e" "<question>" \
