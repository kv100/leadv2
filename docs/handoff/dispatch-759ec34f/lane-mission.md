Product implementation task dispatch-759ec34f. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# ARM-LADDER-KIMI-RESURRECTED-01 — fix round 1

Repo: `/Users/kostiantyn.vlasenko/Projects/leadv2` (the plugin repo — the single source; never
create a real copy of a plugin file inside a project). Read
`/Users/kostiantyn.vlasenko/Projects/persona-engine/docs/handoff/arm-ladder-review-2026-08-06/review-r1.md`
first — it carries the file:line and failure scenario behind every item below.

Do NOT touch the two live persona-engine lanes (`92954b82`, `472b95ed`). Never run
`git reset --hard`, `git clean` or `git stash` anywhere — these are shared trees with live work.

## C1 — BLOCKING. Merge `1b8692e` silently un-retired kimi as a build arm.

Founder order, executed yesterday as commit `3398d11` (DISPATCH-KIMI-ARM-MISMATCH-01): kimi is
retired as a build/dispatch arm. GLM stays, kimi goes. That commit removed kimi from
`glm) candidate_arms=(glm kimi codex sonnet)`.

The merge put it back in two places:
- `plugins/leadv2/config/leadv2-routing.yaml:114-134` — `router.dispatch_ladder` has a live
  `- id: kimi` entry with no `dispatch: false`.
- `plugins/leadv2/scripts/leadv2-dispatch-code.sh:855` — the legacy fallback
  `_LADDER_IDS=(glm kimi codex sonnet)` / `_LADDER_PROVIDERS=(glm kimi codex anthropic)`.

This is live, not theoretical: `plugins/leadv2/scripts/tests/test-routing-enforcement-p1.sh:79-85`
records that an unfaked run during authoring **spawned a real background kimi session**, and Test 1
in that file asserts kimi's presence as intended behaviour.

Required:
1. Retire kimi from the dispatch path via the CONFIG, not a hardcoded exclusion — mark it
   `dispatch: false` the same way `fable` is marked, so the ladder loader skips it. The standing
   rule is *never hardcode an arm out of routing*; the retirement must be expressed as config the
   loader honours. If the loader does not currently honour `dispatch: false` for ladder entries,
   make it do so — that is the real fix.
2. Remove kimi from the legacy hardcoded fallback list at `:855` (both arrays, kept in step).
3. Fix Test 1 in `test-routing-enforcement-p1.sh` — it currently locks in the bug. It must assert
   the opposite: kimi is NOT in the dispatchable build chain.
4. Add a structural guard so this cannot silently regress again: assert that the build-dispatchable
   entries of `_LADDER_IDS` are a subset of the dispatchable build arms known to
   `leadv2-glm-policy-resolve.py`. A drift between the two lists must fail a test, not reach prod.
5. No test in the suite may spawn a real provider session. If any test can reach a live
   `kimi-coder.sh` / `glm-coder.sh` / `codex` invocation, fence it behind a fake or a dry-run flag
   and prove it cannot spawn.

## H1 — the unknown-arm fail-safe was replaced by a WORSE behaviour, not merely removed.

My original suspicion (hard exit on unknown arm) is REFUTED — the review proved
`_load_dispatch_ladder` + `_build_candidate_chain` (`dispatch-code.sh:828-875`) jointly guarantee
`candidate_arms` is never empty, so the `exit 1` guard at `:2976` is dead code.

The real defect: `_build_candidate_chain` ends with
`[[ ${#candidate_arms[@]} -gt 0 ]] || candidate_arms=("${_LADDER_IDS[@]}")` — an arm the ladder does
not know silently falls back to the FULL cheap-first ladder (glm first) with **no journalled
decision**. `3398d11` deliberately made that case fall to sonnet only and emit
`arm_vocabulary_mismatch by=router arm=… fallback=sonnet … reason=launcher_unknown_arm`.

Required: restore the safe + observable behaviour — unknown arm ⇒ sonnet-only chain ⇒ emit the
`arm_vocabulary_mismatch` decision line. Then either delete the now-dead `exit 1` guard at `:2976`
and the dead `_candidate_chain_for_arm` at `:1608-1621`, or wire whichever one you keep; do not
leave two dead fallbacks and one live one.

## H2 — the quota precheck's write side is inert scaffolding.

`_record_quota_lockout` (`dispatch-code.sh:914`) has **zero callers** in the repo. Nothing ever
records a lockout, so the precheck can never fire in production — a feature that reports as shipped
and does nothing. Either wire it to the real lockout signal (the point where a provider returns a
quota/rate-limit refusal) with a test proving a recorded lockout changes the next dispatch's arm, or
delete it and say plainly in the commit message that the precheck is read-only against an
externally-written store, naming that writer.

Ceilings, for reference: glm 80/90, codex 90/95, claude 95; review gets the higher ceiling.

## M1 / M2 / L1 — do them while you are in the file.
- `_quota_locked` (`:~900`) returns 0 for "not locked", inverted from predicate convention. Correct
  at the single current call site, a trap for the next one. Either flip it to convention and fix the
  call site, or rename it so the name states what it returns.
- No test covers "neither project nor plugin routing config exists" (the P3 self-host resolution
  gap). Add one: it must assert the degraded mode announces itself rather than proceeding silently.
- The legacy fallback ladder has no `dispatch:false`-equivalent filter. Once C1 lands, keep the two
  paths consistent.

## Verification — mandatory, no code-reading claims accepted
- Every fix carries a test that **FAILS against current HEAD (`1b8692e`) and passes after**. Show
  both runs verbatim. A test that passes before the fix proves nothing.
- Run the full routing/dispatch test scope and report the counts before and after.
- For C1 specifically: show a dispatch dry-run whose resolved candidate chain does not contain kimi,
  starting from an arm that previously would have included it.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-759ec34f" "<question>" \
