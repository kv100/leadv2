Product implementation task dispatch-78b4d5fd. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BUILDER-SELFCHECK-GATE-01 — NARROW FINISHER r7 (resume lane 9c027877)

Everything is committed on the lane (f2cc0bc + 7ffe67c) except a 1-line edit in
lib/leadv2-builder-selfcheck.sh. Lead ran the lane's run-core-offline solo (08:2x): 50/5.
Two reds are main flakes fixed in ANOTHER lane, NOT yours, ignore them:
"dispatch refusal fallback chain" and "plan-followups-01".

YOUR reds (all interact with your selfcheck gate in leadv2-dispatch-product-close.sh —
this is exactly round-2 finding C3 that was never proven fixed):
  1. "product-close waits for worker exit"  (test-product-close-waits*.sh)
  2. "report-only gate (REPORT-ONLY-GATE-01)"
  3. possibly a 5th suite (runner said failed=5, four names printed — re-run and identify).

## Work (NARROW — nothing else)
1. FOREGROUND-run each failing suite individually in the worktree; capture the exact FAIL
   lines. NEVER background-and-wait a suite (that killed 4 workers already).
2. Root-cause: most likely the selfcheck gate fires inside these suites' product-close
   fixtures (recursion/depth guard C1, or checks=0/degraded path M1, or the gate blocking
   report-only lanes which must fall through by design). Fix in lib/leadv2-builder-selfcheck.sh
   / the gate block in product-close — the gate must be inert for those fixture paths WITHOUT
   weakening what it asserts for real lanes (a suite-only bypass env var = lying-green, FAIL).
3. Re-run the 3 suites + test-builder-selfcheck-gate.sh (must stay green incl. red-first legs).
4. bash -n + shellcheck -S warning on changed files.
5. COMMIT (include the pending 1-line lib edit after auditing it: `git diff` first).

## Off_limits
Everything outside lib/leadv2-builder-selfcheck.sh, leadv2-dispatch-product-close.sh (gate
block only), tests/test-builder-selfcheck-gate.sh. Do NOT touch the two main-flake suites.

## Terminal artifact
Commit sha + per-suite before/after raw output + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-78b4d5fd" "<question>" \
