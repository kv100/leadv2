Product implementation task dispatch-7b9d819b. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# REVIEW-ARM-FAILCLOSED-01\n\nFix a confirmed review-gate control-flow defect in plugins/leadv2/scripts/leadv2-review-run.sh. Evidence: GLM-5.3-ROUTING-FINAL external review launched a failing/empty GLM reviewer arm and no review-<arm>.rc nor review-gate.md was written: _engine_arm_job calls run_reviewer_arm under set -e, so a nonzero arm aborts before persisting its result. Make the gate fail-closed and terminally legible: always persist the arm rc and allow the parent to classify/reroute or write an explicit status; do not silently abort. Add a hermetic regression test that forces a nonzero/empty reviewer arm and asserts the expected gate artifact/status and bounded behavior. Do not run known unsafe dispatch suites. Keep change scoped; run targeted tests and report exact outputs.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-7b9d819b" "<question>" \
