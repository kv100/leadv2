Product implementation task dispatch-7bdb16ee. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# HOOK-INJECT-DEDUP-01 — content-hash gate on per-turn hook injections (autoresearch F6)

Repo: ~/Projects/leadv2. Evidence: `~/Projects/persona-engine/docs/research/token-burn-anatomy-20260823.md`
(marathon lead session: 12,946 hook-injection events ≈ 223K tokens injected — open-threads 121K,
scheduled-decisions 115K, BROAD_STATUS 109K, task-anchor 107K — each injection then re-read every
subsequent turn until compaction) and finding F6 (PARK, now ordered by founder) in
`~/Projects/persona-engine/.claude/autoresearch/sessions/token-economy-2026-08-23/findings.md`.

Gap: the UserPromptSubmit injectors (`plugins/leadv2/hooks/leadv2-task-anchor.sh` and the
sibling injectors that emit open-threads / scheduled-decisions blocks — enumerate them from
hooks.json UserPromptSubmit entries) re-emit their full block EVERY turn with no
"unchanged since last injection" gate.

## Deliverables
1. Per-injector last-injected content hash in a small state file (keyed by session_id, under
   $HOME/.claude/state/leadv2/ — mirror how sibling hooks persist per-session state, e.g. the
   turncap STATE_DIR pattern). On each firing: build the would-be injection, hash it; if
   identical to last injected hash for this session → emit a ONE-LINE marker instead
   (e.g. `[task-anchor unchanged]`) or nothing (pick per injector: task-anchor gets the 1-line
   marker so the anchor reference survives; pure-informational digests get nothing).
2. URGENCY EXCEPTION (from F6 risk note): scheduled-decisions must NOT be suppressed when a
   row's Due-state changed only by time passing (newly DUE/OVERDUE with no byte change). Key its
   hash on (content + today's date + the computed DUE/OVERDUE classification), so a date flip
   re-injects.
3. BROAD_STATUS relay lines are NEVER deduped (they are the founder's pulse; each beat is new).
   Exclude that injector entirely.
4. Env kill-switch LEADV2_INJECT_DEDUP=0 restores current behavior; default ON (this one is
   harness-side and does not block founder prompts — the 2026-08-19 "never block a founder
   prompt" order does not apply to suppressing REDUNDANT injections, only to blocking).
5. Tests: unchanged content → marker/nothing on 2nd fire; changed content → full re-inject;
   date-flip re-inject for scheduled-decisions; kill-switch; fail-open (state dir unwritable →
   full inject as today).
6. Docs paragraph beside the hooks docs.

Constraints: minimal diffs per injector; never edit outside ~/Projects/leadv2; bash -n + tests.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-7bdb16ee" "<question>" \
