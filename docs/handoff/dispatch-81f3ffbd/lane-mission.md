Product implementation task dispatch-81f3ffbd. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# WORKER-CONTEXT-DIET-01 — fix round 1 (critic BLOCK findings, narrow finisher)

Worktree already exists: /Users/kostiantyn.vlasenko/Projects/leadv2/.claude/worktrees/9341e2eb
(branch worktree-9341e2eb). Work ONLY there. Mechanical fixes only — no refactors.

Context: after the live probe measured cache_creation delta ≈ 0, the lead flipped both env gates
to default OFF (commit e908a5f): LEADV2_SUBSESSION_SLIM_MCP:-0 and
LEADV2_SUBSESSION_EXCLUDE_DYNAMIC:-0, and adjusted tests CD-01..06 to pass =1 explicitly.
Critic then BLOCKed with these findings — fix each:

1. [CRIT] No test now covers the SLIM_MCP default-unset path (all CD-01..05 export =1). Add one
   test to plugins/leadv2/scripts/tests/test-subsession-context-diet.sh: with BOTH envs unset,
   the spawn must contain NO --strict-mcp-config, NO --mcp-config, NO
   --exclude-dynamic-system-prompt-sections, and emit no context-diet WARN lines.
2. [MED] claude-subsession.sh:515 (approx) — an unguarded write that breaks the WARN contract
   when the handoff/dir is missing: find the resolve_role_mcp_config()-adjacent write the critic
   means (any redirect/file-write not guarded by mkdir -p or 2>/dev/null fail-open) and guard it
   so a missing directory can never kill a spawn or swallow the WARN. Read the function; the
   defect is visible on inspection.
3. [MED] Docs drift: plugins/leadv2/docs/context-diet.md lines ~10/~63/~79-80 and
   plugins/leadv2/docs/phases.md line ~478 still describe the defaults as ON (1). Update to
   default OFF / opt-in =1, and state the reason in one line: live probe 2026-08-23 measured
   delta ≈ 0, mission gate "delta <10K → no default-on".
4. [MED] Asymmetric gate semantics: SLIM_MCP uses `:-0` != "1"` (any non-1 = off) while
   EXCLUDE_DYNAMIC uses `:-0" != "0"` (any non-0 = ON, so =2 enables). Normalize BOTH to strict
   opt-in: enabled iff the value is exactly "1". Update the comment at the EXCLUDE_DYNAMIC site
   ("Only the literal 0 disables") to match, and any test that relied on loose parsing.
5. [LOW×2] The critic's two Low findings are unknown to the lead (its full text was lost) — if
   `git log`/comments in the worktree don't reveal them, skip; do NOT invent work.

Then: bash -n on touched files; run test-subsession-context-diet.sh (all pass, including the new
default-unset case); commit in the worktree naming 9341e2eb.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-81f3ffbd" "<question>" \
