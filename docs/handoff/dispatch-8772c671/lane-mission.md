Product implementation task dispatch-8772c671. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# QUOTA-GATE-PARITY-01 — fix round 2 (review verdict FAIL, 7 High)

Repo: ~/Projects/leadv2, lane worktree .claude/worktrees/b413968c (continue on the existing
branch — do NOT recreate it). Your prior diff is committed as bef2b86 and reviewed.

Fix ALL of these review findings (full bodies: docs/handoff/dispatch-b413968c/review-codex.md,
review-glm.md, review-opus.md; gate summary: review-gate.md):

1. [High] lib/leadv2-glm-policy-resolve.py:335 — top-level Codex `limit_reached=true` is
   ignored when the binding window exists without its own true flag → resolver can pick a
   provider already hard-limited. Honor the top-level flag as an OR.
2. [High] lib/leadv2-codex-quota-gate.sh:68 — the unconditional live gate makes core offline
   quota-guardrail tests depend on the HOST's real Codex quota state. Tests must run hermetic:
   respect a test env override (existing idiom: LEADV2_*_BIN / fixture JSON path) so suites
   never read the live cache.
3. [High] leadv2-glm-quota-gate.sh:44 — nonnumeric LEADV2_CEIL_GLM_WORK reaches arithmetic
   under set -u and aborts the gate. Validate numeric, else fall back to default (fail-open
   contract).
4. [High] leadv2-provider-quota-gate.sh:28 — unbounded LEADV2_QUOTA_READ_TIMEOUT can block
   every spawn. Clamp to a sane max (e.g. 60s).
5-7. [High] evidence-contract: the claim "Codex limit_reached means 100%" drives decisions in
   provider-quota-gate.sh:61, burn-governor.sh:217, glm-policy-resolve.py:335 but has no probe
   artifact. Either add the probe artifact reference (the quota-cache codex.json carries
   `limit_reached` + `used_percent` from the live API — cite the fetcher + a captured sample in
   a comment) or tag the mapping UNVERIFIED and make the code treat limit_reached as a
   standalone block signal rather than synthesizing "100%".

Also: the review's verify pass wrote empty files (0/7 verified) — that is the reviewer's
infrastructure, not your concern; do not touch review scripts in this lane.

Deliverables: fixes + updated tests covering 2,3,4 (hermetic; nonnumeric ceiling; timeout
clamp) + bash -n + suite green. Commit on the lane branch. HARD RULE: no halting on prepass
mismatches; proceed and note in summary.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-8772c671" "<question>" \
