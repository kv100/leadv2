Product implementation task dispatch-8e705910. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# LANE-REGISTRY-SELF-DEADLOCK-01 — dispatcher cannot re-dispatch a task whose worker died

Repo: ~/Projects/leadv2. Founder order 2026-08-24 («тоже надо запустить и сделать»).
Live incident: EGRESS-STATUS-COLLECTOR-01 (task `e5be9e72`) could not be re-dispatched no
matter how many times; had to be routed around via codex-task.sh. Full incident record:
~/Projects/persona-engine/docs/leadv2/open-threads.md (search LANE-REGISTRY-SELF-DEADLOCK).

Scope: `plugins/leadv2/scripts/leadv2-dispatch-code.sh` (+ any helper it sources for lane
liveness). Two defects. A third failure mode (SessionStart worktree sweep) is EXPLICITLY OUT
OF SCOPE — it is being fixed in parallel lane SWEEPER-LANE-SAFETY-01; do not touch the sweep
hooks.

## Defect 1 — active.yaml registers the LEAD's PID, not the worker's
The lane registration writes the calling (lead) session PID into docs/leadv2/active.yaml
(observed: pid 13036 = the lead `claude --dangerously-skip-permissions`, process birth time
matched exactly). The liveness probe then sees a process that is alive by definition — the
lane can NEVER be reclaimed while the lead session lives.
Fix: register the SPAWNED WORKER's PID (the handle the dispatcher itself prints as
`worker_spawned ... handle=PID=<n>`), captured AFTER the spawn, not the dispatcher's own
$$/PPID. If registration must happen pre-spawn, update the row post-spawn with the real PID.
Liveness must additionally verify process identity (start-time or command-line match), not
bare `kill -0`, so a recycled PID can't fake liveness.

## Defect 2 — `prepass_stream_fresh` probe is refreshed by the refusal path itself
After active.yaml is cleared, the liveness fallback checks mtime freshness of
`docs/leadv2/tasks/dispatch-<id>/journal.md` — but every REFUSED re-dispatch attempt WRITES
that journal, refreshing the very mtime that causes the refusal (`verdict=starting:80` →
`starting:146`, age growing with each retry). A retry loop can never converge.
Fix: the freshness probe must only consider WORKER-authored signals (the worker stream
`developer.stream.jsonl` mtime, or journal lines tagged by the worker attempt id) — never
files the dispatcher/refusal path itself writes. Simplest robust shape: probe the lane's
stream file + worker PID identity from Defect 1; drop journal.md from the liveness signal
entirely.

## Deliverables
1. Both fixes in leadv2-dispatch-code.sh (and sourced helpers), minimal diff.
2. Journal one line on every reclaim decision: `lane_liveness verdict=<live|dead> task=<id>
   signal=<pid_identity|stream_fresh|none>` so the next forensic session sees WHY.
3. Tests (fixtures, no real spawns): (a) row with lead PID + dead worker → reclaimable;
   (b) recycled-PID row (PID alive but different identity) → reclaimable; (c) refused
   attempt does NOT refresh any file the liveness probe reads (probe verdict identical
   before/after a refusal); (d) genuinely live worker (mock stream fresh + matching PID
   file) → refused as live.
4. bash -n + existing dispatcher test suite green (bash 3.2 compatible).

## HARD RULE (anti-halt)
Do NOT halt on prepass/census mismatches (line numbers, helper names, counts). The prepass
is advisory; this mission + the live incident record are authoritative; your fixture tests
are the arbiter. On mismatch: one summary line, then PROCEED to implement.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-8e705910" "<question>" \
