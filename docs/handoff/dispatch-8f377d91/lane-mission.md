Product implementation task dispatch-8f377d91. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# V3-GLM-LADDER-01 — ROUND 2 FINISHER (lane eb2d7143, on top of commit 389820a)

Round-1 verdict FAIL (diff=9ae3650e, reviewer=opus-critic). READ the full report FIRST:
~/Projects/leadv2/docs/handoff/dispatch-eb2d7143-review/critic.full.md. Fix C1-C3, H1-H3, M1-M5
(L1-L4 optional). Summary:

C1 dispatch-code.sh:4402-4412 — park+counter fire only on the FIRST refusal; afterwards the quota
   lockout benches glm out of candidate_arms entirely (attempted[] has no glm_refused_*), so
   count=1 forever. Fix: also park/count when glm is skipped via quota_precheck_skip
   reason=provider_quota_locked. Your own suite must reproduce this through ONE shared cache dir
   (the r1 harness hid the bug by giving each run a fresh cache dir — that is a lying-green
   harness; fix the harness too).
C2 :742,:906-923 — --retry-all can never retry a real row: park rows get mission_path="" (mission
   file only exists for product class, written later at spawn), and the landed sonnet fallback
   makes dispatch_terminal_exists true → already_terminal forever, row never reaped. Fix: park the
   MISSION CONTENT (or the @file the founder passed) at refusal time into the park row (copy into
   docs/leadv2/glm-deferred.d/<sig8>.md), and retry must dispatch as a NEW task (new sig8) derived
   from the parked mission, not re-check the old task's terminal.
C3 test-glm-deferred-ladder.sh — add real --retry-all coverage: park a row with a real mission,
   retry, assert a new dispatch attempt happens (fixture launcher) and the row is reaped.
H1 .gitignore rules for the park/lock files were never committed — commit them in this lane.
H2 :773,934,971,1007 — replace 4 bare `flock 8` with the repo's lv2_lock_wait (macOS has no
   flock(1); proven to run unlocked otherwise).
H3 :920-937 — mark-retried must happen only after a successful dispatch (check rc) and inside the
   lock; a failed retry leaves the row parked.
M1 remove dead _glm_deferred_is_retried (or use it); M2 reason field carries the actual
   glm_refused_* variant; M3 park gate = quota refusals only, not any refusal; M4 counter counts
   distinct sig8/day; M5 fix the 3 SC2034 locals.

## Acceptance
- Suite green (FAIL=0) INCLUDING: one-shared-cache-dir two-refusal case (count=2, 2 park rows or
  1 row + dedup — pick semantics and assert it) AND a real --retry-all leg.
- bash -n + shellcheck -S warning clean on changed files (no new SC2034).
- test-lane-placement-pin.sh 24/0 (dispatch path untouched invariant).
- .gitignore committed with the park/lock patterns.
- COMMIT on lane branch.

## Off_limits
Routing order/ceilings unchanged; no new env vars; leadv2-dispatch-product-close.sh; supervise*.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-8f377d91" "<question>" \
