Product implementation task dispatch-908164a1. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# PULSE-WAKE-01 — the 30-min pulse must ALWAYS reach the founder chat (founder order 2026-08-24)

Repo: ~/Projects/leadv2. Founder verbatim: «надо раз и навсегда починить чтобы инфа всегда раз
в 30 мин приходила... и не было такого (никогда) чтобы молчание долгое».

Problem: the plugin writes founder-status.md + emits BROAD_STATUS_READY every ~30 min, but
delivery to chat needs the lead session to take a TURN (founder message or Monitor event).
An idle lead session never relays — beats silently rot. Today: 2h of founder-visible silence.

The mechanism that works (proven live in the lead session right now): a persistent Monitor
watching founder-status.md mtime; on each beat it emits the file's first line as an event →
the lead gets a turn → the UserPromptSubmit hook injects the RELAY directive → the lead
relays. Make this canonical and automatic:

## Deliverables
1. `plugins/leadv2/scripts/leadv2-pulse-watch.sh` — prints the canonical watcher command
   (single line, bash 3.2, no deps beyond stat/sleep) and, with --emit-loop, IS the loop:
   watch founder-status.md mtime (path via the state-path resolver, NOT hardcoded), on change
   print line 1 of the file, sleep 60. Skip the very first observation (arming is not a beat).
2. SessionStart injection: extend the existing single-lead SessionStart hook (or add a small
   one) so every LEAD session (not workers — reuse the existing lead-vs-worker discrimination
   the hooks already do) sees one line: "PULSE-WAKE: arm the beat watcher NOW:
   Monitor(<command from leadv2-pulse-watch.sh>)" — same idiom as existing directives.
3. `docs/single-lead-pulse.md`: replace the passive "relay when a beat arrives" wording with
   the armed-watcher contract: the lead arms the watcher at session start; a session without
   the watcher armed is in violation; beat event → relay per existing RELAY=full/none rules.
4. Tests: leadv2-pulse-watch.sh --emit-loop against a fixture file (touch it → one line out;
   no touch → no output; first observation suppressed); bash -n; hook test extended if the
   suite covers SessionStart output.
5. Do NOT build a daemon/cron; the Monitor lives inside the lead session by design (its
   events create the turns). Keep the diff minimal.

HARD RULE: no halting on prepass/census mismatches — probe, one summary line, proceed.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-908164a1" "<question>" \
