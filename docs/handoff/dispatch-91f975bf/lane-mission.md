Product implementation task dispatch-91f975bf. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BROAD-STATUS-RELAY-SCOPE-01 — full BROAD_STATUS only to the owning session (SHARED edit, founder-approved 2026-08-19)

## Outcome (one)
The BROAD_STATUS_READY beat injects the FULL founder-status.md relay directive only into the
session that owns the beat (the session running supervise, or the session owning active lanes);
every OTHER live lead session gets exactly ONE line (stamp + path + "full status in owning
session"), never the full ledger dump.

## Premise
Founder hit this today: a focused session received the verbatim 25-row dump of other sessions'
ledger. Founder decision recorded in persona-engine docs/leadv2/open-threads.md ("DECIDED:
SHARED edit (option a)").

## Where (trace, then edit the live reader)
Find the emitter that writes the BROAD_STATUS_READY line into session prompts (UserPromptSubmit
hook / supervise beat renderer in plugins/leadv2/hooks/** or scripts). Determine "owning session"
from what the beat already knows: the session id that armed the beat / renders founder-status.md
(registry in docs/LEAD_V2_STATE.md sessions map or active.yaml). Fallback fail-open: if ownership
cannot be resolved, keep today's behavior (full relay) — never silently drop the beat.

## Write set
plugins/leadv2/hooks/** · plugins/leadv2/scripts/** (beat/status-surface renderer) ·
plugins/leadv2/scripts/tests/** (new suite).

## Non-goals / off_limits
- leadv2-dispatch-product-close.sh and leadv2-review-run.sh (other lanes own them today).
- Do not change founder-status.md CONTENT or the beat cadence — only who gets the full relay.
- IMPORTANT: if you change a HOOK file, state it loudly in the deliverable — hooks live in a
  plugin CACHE copy and need copy+session-restart to go live (known trap).

## Acceptance
- Test: beat with owning-session context → full relay directive; non-owning session → single
  line; unresolvable ownership → full relay (fail-open). Red-first, pinned baseline.
...
## Terminal artifact
Commit + sample of both injected lines (owning vs non-owning) + note whether a hook-cache copy
is required + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-91f975bf" "<question>" \
