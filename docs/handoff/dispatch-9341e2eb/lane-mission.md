Product implementation task dispatch-9341e2eb. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# WORKER-CONTEXT-DIET-01 — slim headless worker context + earlier lead compaction

Repo: ~/Projects/leadv2 (the plugin repo). Grounded in two evidence docs — read both first:
- ~/Projects/persona-engine/docs/research/token-burn-anatomy-20260823.md (measured burn profile)
- ~/Projects/persona-engine/.claude/autoresearch/sessions/token-economy-2026-08-23/findings.md
  (F1/F2/F3 are the findings this task implements; every CLI flag there was verified against
  installed claude --help v2.1.241)

## Problem (one line)
Every headless worker spawn pays ~90–110K tokens of MCP tool schemas + skill catalog it never
uses (63–76K cache_creation fresh per spawn, 302 spawns/day ≈ 1B/day of read-echo), and the
interactive lead session compacts only at 400–500K width (~70–80% of its 513M/day is stale
reread).

## Deliverables

### 1. `plugins/leadv2/scripts/claude-subsession.sh` — flag-gated slimming (F1+F2)
In the CLAUDE_ARGS block (~line 405):
- `LEADV2_SUBSESSION_SLIM_MCP` (default **1**, kill-switch 0): append
  `--strict-mcp-config --mcp-config <role-config>` where role-config is a NEW per-role JSON at
  `plugins/leadv2/config/mcp-role-<ROLE>.json` (create for developer/critic/architect + a
  `default` fallback used for any other role). Contents: ONLY the MCP servers that role's agent
  frontmatter tools list actually references — in practice repowise only (check
  `.claude/agents/{developer,critic,architect}.md` tools lists in a target repo; they reference
  `mcp__repowise__*` and for developer also `mcp__codebase-memory-mcp__*` which is DISABLED in
  persona-engine — include repowise only, note the graph-MCP omission in a comment). The
  repowise server command/config must be copied from what a target repo's .mcp.json /
  enabledMcpjsonServers actually uses — resolve it dynamically if per-repo (inspect how repowise
  is registered: `~/Projects/persona-engine/.mcp.json` or `.claude/settings.json`); if the
  server definition cannot be resolved at spawn time, FAIL OPEN: skip the two flags entirely and
  log one WARN line (a worker with fat context beats a worker without repowise).
- `LEADV2_SUBSESSION_EXCLUDE_DYNAMIC` (default **1**, kill-switch 0): append
  `--exclude-dynamic-system-prompt-sections` (moves per-machine cwd/git-status sections out of
  the cached system-prompt block → cross-spawn cache-prefix reuse). Verify the mission body
  already restates cwd + base SHA (single-lead-mode mandates base SHA); if not, add one line to
  the per-task boilerplate so workers keep situational awareness.
- Do NOT use `--bare` (it forces API-key auth, silently leaving the subscription pool — trap
  documented in findings F1). Do NOT use `--agent` in this task (cache-prefix interaction
  unproven — leave a TODO comment referencing findings F1).

### 2. Measurement probe (proof, not vibes)
New `plugins/leadv2/scripts/leadv2-context-diet-probe.sh`: spawns two trivial one-turn workers
per config (flags on / flags off; role critic; mission "reply DONE") via claude-subsession.sh
in a scratch dir, parses each stream-json for first-turn `cache_creation_input_tokens` +
`cache_read_input_tokens`, prints a 4-row table and the delta %. This is the close evidence:
run it and paste the table into the close notes. Expected: on-config base drops by tens of K;
if delta < 10K, say so honestly — do not ship default-on (flip both defaults to 0 and report).

### 3. Lead compaction (F3)
- Investigate (installed claude v2.1.241): can `--autocompact <tokens>` be set via settings.json
  (autocompact key? env?) so it applies to the founder's interactive sessions without changing
  how he launches claude; verify it survives `--continue/--resume`.
- Wire the best available mechanism at 150000 for lead sessions: settings-level if supported
  (repo .claude/settings.json of the 3 live repos is NOT plugin-owned — in that case deliver the
  exact one-line change as a doc note + add it to plugin docs), or a documented launch wrapper.
  If ONLY a flag exists and settings can't carry it, do not invent a wrapper the founder won't
  use — document precisely and stop.

### 4. Tests + docs
- Unit test for the role-config resolution (existing tests/ conventions): role→file mapping,
  fail-open on missing config, kill-switch envs.
- Docs: short section in the plugin docs beside the other LEADV2_SUBSESSION_* knobs; both new
  envs + the probe script + the --bare trap warning.

## Constraints
- Fail-open everywhere; a worker must never lose repowise or die because a config file is
  missing.
- Keep diffs localized; claude-subsession.sh is load-bearing for every dispatch arm.
- Never edit outside ~/Projects/leadv2.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-9341e2eb" "<question>" \
