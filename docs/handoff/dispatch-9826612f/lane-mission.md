Product implementation task dispatch-9826612f. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CODEX-LEAD-FULL-01 — full-fledged Codex-lead tooling (founder: "сделать сразу полноценно")

Repo: ~/Projects/leadv2. Context (read first): plugins/leadv2/docs/codex-lead-pilot-runbook.md,
plugins/leadv2/docs/codex-lead-AGENTS-pilot.md, ~/.codex/prompts/leadv2.md (existing shim),
plugins/leadv2/hooks/hooks.json (what the Claude-side blocking hooks enforce).

Goal: Codex has no PreToolUse/blocking hooks, no plugin system, no statusline. Build the
closest FULL equivalent with what Codex does have: custom prompts (~/.codex/prompts/*.md,
$ARGUMENTS supported), AGENTS.md prose, and our own bash. Everything ships in the plugin repo
under plugins/leadv2/codex-lead/ plus an installer.

## Deliverables

1. **Prompt pack** `plugins/leadv2/codex-lead/prompts/` (installed to ~/.codex/prompts/):
   - `leadv2.md` (upgrade of the existing shim — session bootstrap: read brief, tails, quota)
   - `leadv2-status.md` — render the founder status: quota-live report + founder-status.md +
     active lanes from active.yaml, as ONE compact Russian table (mirror single-lead-pulse.md
     format).
   - `leadv2-dispatch.md` — guided dispatch: premise-check the task, then call
     leadv2-dispatch-code.sh via lv2guard (below); explain exit codes incl. 6=burn-park.
   - `leadv2-review.md` — run leadv2-review-run.sh for a lane, interpret verdict, fix-round
     protocol (max rounds now enforced engine-side).
   - `leadv2-close.md` — close ritual: evidence-based close, journal append, ledger row for
     anything deferred, worktree left on disk.
2. **Guard wrapper** `plugins/leadv2/codex-lead/lv2guard.sh` (installed onto PATH or invoked by
   relative path): `lv2guard <command...>` — bash reimplementation of the CATASTROPHIC-tier
   deny-floor: refuse rm -rf on $HOME/shared trees, git reset --hard/clean/stash in
   ~/Projects/* shared checkouts, git worktree prune while active lanes exist (check
   active.yaml), direct `codex exec` (must go through codex-task.sh), heredoc>2KB advisory.
   Exit 97 with a loud reason on refusal; exec the command otherwise. The AGENTS brief must
   mandate: every shell command with side effects goes through lv2guard (prose rule — the
   pilot measures compliance).
3. **Statusline substitute**: `plugins/leadv2/codex-lead/leadv2-codex-status.sh` — one-line
   output equivalent to the Claude statusline (cc/cx/glm % + lanes + active task), used by
   leadv2-status prompt and callable directly.
4. **Installer** `plugins/leadv2/codex-lead/install.sh`: idempotent — copies prompts to
   ~/.codex/prompts/, ensures repowise MCP block in ~/.codex/config.toml (don't duplicate),
   verifies the AGENTS.md @import line exists in the target repo (arg: repo path, default
   persona-engine), prints what it did. Never overwrites user customizations silently
   (back up changed files to *.bak).
5. **AGENTS brief upgrade**: extend codex-lead-AGENTS-pilot.md with the lv2guard mandate +
   the prompt-pack usage map (keep ≤150 lines total).
6. Tests: lv2guard deny/allow cases (fixture dirs, no real destruction); installer idempotency
   (run twice → no dupes); bash -n everything.
7. Docs: extend codex-lead-pilot-runbook.md §Install with `install.sh` one-liner.

Constraints: never edit outside ~/Projects/leadv2 (installer RUNS elsewhere but ships here);
minimal deps (bash+jq+python3); fail-open on missing telemetry, fail-CLOSED in lv2guard on its
deny rules.

## HARD RULE (lead amendment, 2026-08-24, after two prepass halts)
Do NOT halt on prepass/census mismatches (rule counts, file lists, regex behavior claims).
The prepass is advisory; THIS mission file is authoritative; your own fixture tests are the
arbiter. On any mismatch: write one line about it in the summary and PROCEED to implement.
A third halt-without-code is a failed attempt, not caution.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-9826612f" "<question>" \
