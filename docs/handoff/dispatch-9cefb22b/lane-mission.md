Product implementation task dispatch-9cefb22b. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
CLAUDE-MULTIPROFILE-QUOTA-02: implement portable opt-in Anthropic multi-profile dispatch for this plugin. No hardcoded account name, email, filesystem path, OAuth token, or machine-specific default may enter the plugin. Define a documented user-level registry/config that can name two or more Claude profiles. Before each Claude lane, independently read each configured profile availability or quota when supported, choose the healthiest profile, and pass only its selected CLAUDE_CONFIG_DIR to the child process. Preserve single-profile fallback; record only a non-sensitive profile label and evidence in handoff. Add hermetic tests and plugin cache validation. No application changes.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-9cefb22b" "<question>" \
