Product implementation task dispatch-9ea4cd03. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# MERGED-BATCH-FIXROUND-01 — fix the 6 High from the lead critic before push

Repo: ~/Projects/leadv2 — work in YOUR OWN lane worktree (dispatcher provides it); the fixes
land on your branch and the lead merges. Authoritative findings report (read FIRST, full
bodies with reproduction commands):
/Users/kostiantyn.vlasenko/Projects/leadv2/docs/handoff/dispatch-merged-batch/critic-lead.md

Fix ALL six High findings:
- H1 deny-extra.yaml: plugin self-removal floor bypassable (two one-liner shapes named in
  the report) — extend the rule(s) to cover them; add deny tests for each bypass shape.
- H2 leadv2-merged-worktree-sweep.sh:166,171,189: `worktree remove --force` destroys
  deliverables where plain remove would refuse — drop --force on the sweep path (or gate it
  behind the protection lib verdict); test that a dirty-but-unprotected worktree is refused,
  not destroyed.
- H3 test-merged-sweep-orchestration-dirt.sh case_newborn_kept is a tautology (never reaches
  the newborn guard) — rewrite the case so the newborn guard is actually exercised (report
  explains why it short-circuits).
- H5 leadv2-fanout.sh / leadv2-fanout-lane-launcher.sh: resolution order reversed against the
  documented LEAD-CONTROL-PLANE-01 invariant (vendored copy preferred BEFORE sibling for the
  control-plane resolution) — reconcile per the report's proposed chain + add the property
  test the report names. NOTE these files are currently UNCOMMITTED in the main checkout
  (a codex worker's aborted edits) — your fix supersedes them; implement in your worktree
  from origin state + the report's recommendation.
- H6: new live-path modules (protection lib / provider gate — per report) shipped without
  behavioural tests on the named properties — add the specific missing behavioural tests.
- H4 is for the LEAD (review artifact completeness), not you — skip; do not rely on
  review.diff, work from the report + live files.

Medium/Low from the report: fix any that are ≤5 lines each and in files you already touch;
list the rest untouched in your summary.

bash 3.2; suites green (targeted + the ones you extend); commit per-finding or grouped
logically. HARD RULE: no halting on prepass/census mismatches — probe, note, proceed.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-9ea4cd03" "<question>" \
