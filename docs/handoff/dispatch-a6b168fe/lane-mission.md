Product implementation task dispatch-a6b168fe. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
CODEX-PULSE-HOOK-02: after transport repair feb253b, implement the smallest honest Codex-native founder Pulse delivery. It must show changed/active status in the lead session at the requested cadence when the supported Codex lifecycle permits it. If an idle chat injection is impossible, do not fake a timer: implement the strongest supported trigger and document the boundary. Include hermetic tests, marketplace/cache validation, a concise handoff, and no application changes.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-a6b168fe" "<question>" \
