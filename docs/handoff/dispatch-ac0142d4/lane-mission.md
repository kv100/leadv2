Product implementation task dispatch-ac0142d4. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CORE-OFFLINE-WORKTREE-GAP-01 — e2e suite red in every lane worktree, green on main

Repo: ~/Projects/leadv2. Today ALL 5 lanes died at the e2e gate on
tests/run-core-offline.sh → "fanout classifier/runner guard", while the same suite on the
main checkout is green (exit 0). Diagnosed cause (verify, then fix):

leadv2-fanout.sh:49-52 resolves the active-registry helper as
  1) ${PROJECT_ROOT}/.claude/scripts/leadv2-active-registry.sh
  2) ${HOME}/.claude/leadv2-shared/scripts/leadv2-active-registry.sh
`.claude/scripts/` is an UNTRACKED symlink farm that exists only in the main checkout — a
lane worktree does not have it, and the test's fixture HOME does not stage the shared copy
either, so in any worktree the source at line 52 hits "No such file or directory" and the
guard test fails. The failure has nothing to do with the lane's own diff — it killed lanes
whose diffs never touch fanout.

## Deliverables
1. Fix so the suite is green in BOTH environments (main checkout AND a lane worktree):
   - fanout.sh: add the canonical resolution idiom BEFORE the HOME fallback — resolve via
     its own SCRIPT_DIR sibling (plugins/leadv2/scripts/leadv2-active-registry.sh if the
     canonical copy lives there) and/or LEADV2_CANONICAL_ROOT — matching how other scripts
     in this repo already resolve shared helpers; AND
   - the fanout guard test: stage the registry helper into its fixture explicitly so the
     test is hermetic and never depends on the host's untracked symlink farms.
2. Census: grep the OTHER core-offline fixtures/tests for the same class (a source path
   that exists only via untracked .claude/scripts or $HOME) — fix any same-class instance
   or state in the summary why each is safe. Also check test "deferred-GLM ladder
   (V3-GLM-LADDER-01)" which failed for lane 6321b955 — if it has the same env leak
   (reads live quota/burn state), make it hermetic with the existing test-env override idiom.
3. Proof: run tests/run-core-offline.sh from INSIDE your lane worktree (that IS the failing
   environment) — paste the fanout-guard + ladder lines and final Results in the summary.
4. bash -n; minimal diff.

HARD RULE: no halting on prepass/census mismatches — probe, one summary line, proceed.

## Redispatch note: codex arm produced nothing (16:17Z); rerouting to GLM.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-ac0142d4" "<question>" \
