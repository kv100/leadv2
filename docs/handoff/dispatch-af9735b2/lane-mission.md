Product implementation task dispatch-af9735b2. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# V3-GLM-LADDER-01 — ROUND 3 FINISHER (resume lane eb2d7143, on top of 389820a + uncommitted r2)

The r2 worker finished its edits (uncommitted, 47 files in worktree) but the dispatcher's
e2e gate FAILED: run-core-offline suites passed=52 failed=2. Start with `git status --short`
+ `git diff --stat`; FINISH, do not redo. The r2 findings spec (C1-C3, H1-H3, M1-M5) is at
/private/tmp/claude-503/-Users-kostiantyn-vlasenko-Projects-persona-engine/cd6c46e0-7993-40b2-9ee9-9727c4eb431b/scratchpad/mission-v3-glm-ladder-r2.md
and the full critic report at docs/handoff/dispatch-eb2d7143-review/critic.full.md (canonical
repo). Verify each r2 finding is addressed in the on-disk diff; complete any that is not.

## The 2 red suites (gate log: docs/handoff/dispatch-8f377d91/e2e-gate.log in canonical repo)

RED-1 "dispatch refusal fallback chain" = test-routing-enforcement-p1.sh. FAILs:
  FAIL: quota refusal advances chain -- rc=4 ... dispatch_classified class=non_product reason=explicit_mission_fast_path kind=unknown
  FAIL: codex dead-arm no_first_byte spill -- rc=4 (same shape)
  FAIL: quota lockout write side -- rc=4 (same shape)
LEAD-VERIFIED FACTS you must use:
  (a) This suite is ALSO red on canonical main (b9959aa) — at least the "quota lockout
      write side" case — so part of this is a MAIN regression, not your lane's.
  (b) On main the failure is NONDETERMINISTIC run-to-run: the same fixture task
      (52a91b29) went route_headroom→sonnet-POISON→rolled_back on one run and
      dispatch_classified fast_path rc=4 on the next run — state leaks between runs
      (dispatch ledger dedup / lockout files / park rows keyed on fixed task sigs).
  Root-cause BOTH layers: (1) why fixture missions now classify
  explicit_mission_fast_path kind=unknown → rc=4 (suspects: the CLAIM-EVIDENCE
  evidence-contract mission injection landed in 210a439, or your own park/mission-copy
  changes shifting classification input); (2) the cross-run state leak (the suite must
  isolate LEADV2_DISPATCH_CACHE_DIR / ledger per run — if it already does, find what
  still leaks). Fix code if code is wrong, fix the suite if the suite leaks state —
  with evidence for whichever verdict you reach, per case.

RED-2 "plan-followups-01" = test-plan-followups-01.sh. UPDATED LEAD FACT (08-20 07:0x):
red on canonical main too WHEN RUN UNDER run-core-offline.sh (solo clean run, 49/2), yet
GREEN standalone on the same main (21/0). So it is a harness-context flake (run-order or
shared-state under the runner), NOT a lane regression. Root-cause the harness interaction;
fix the suite's isolation (or the runner) so it is green under run-core-offline.

NOTE: your prior r3 attempt (worker died at acceptance wait) left good work, already
checkpoint-committed by the lead as e9bbd0f — build on it, do not redo. Also: do NOT
launch run-core-offline in the background and idle-wait on it (that is exactly how the
last two workers died of worker_timeout). Run it FOREGROUND and keep working only after
it returns.

## Acceptance
- run-core-offline.sh forward GREEN (FAIL=0) in the lane — including the two suites above.
- The r2 acceptance still holds: one-shared-cache-dir two-refusal case + real --retry-all
  leg in test-glm-deferred-ladder.sh; test-lane-placement-pin.sh 24/0; .gitignore committed.
- bash -n + shellcheck -S warning clean on changed files.
- COMMIT on lane branch.

## Off_limits
Routing order/ceilings unchanged; no new env vars; leadv2-dispatch-product-close.sh;
supervise*; lib/leadv2-builder-selfcheck.sh (another live lane owns it).

## Terminal artifact
Commit sha + per-finding note (incl. RED-1 verdict: main-regression vs lane vs suite-leak,
with raw probes) + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-af9735b2" "<question>" \
