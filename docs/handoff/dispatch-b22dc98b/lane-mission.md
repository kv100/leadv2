Product implementation task dispatch-b22dc98b. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BURN-GOVERNOR-01 — 24h token-burn governor wired into the leadv2 dispatcher

Repo: ~/Projects/leadv2 (the plugin repo itself). All paths below are relative to
`plugins/leadv2/`.

## Why (evidence, 2026-08-23)

Token burn doubled since ~Aug 14: 7d=8.72B vs ~4.5B prior week; today 1.72B and rising.
Measured from `~/.claude/burn/history.db` (sqlite):
- 97% of burn is cache reads → cost = turns × context width.
- Driver #1: headless worker spawn volume tripled. Dispatch lanes/day: 43 (Aug 13) →
  111–192 (Aug 14–17) → 158–162 (Aug 22–23). Today alone: 87 developer sessions (706M),
  104 critic (233M), 111 architect (183M) = 1.23B headless, running 24/7 including 00:00–05:00.
- Driver #2: marathon interactive lead sessions. One session today: 1699 turns, 227 founder
  messages, 513M tokens, 300K cache-read/turn. The 80-turn /compact rule is advisory only.
- Worker base context is ~106K at turn 1 (~150K/turn average), ~4–8M per worker session.

There is NO burn-aware gate anywhere: the dispatcher checks provider quota % but never local
24h burn. The machine happily triples its own fan-out.

## Deliverables

### 1. NEW `scripts/leadv2-burn-governor.sh`
Single-purpose verdict script.
- Subcommand `verdict` (also the default with no args): print exactly one line to stdout:
  `verdict=<ok|soft|hard> burn24h=<int> soft=<int> hard=<int> reason=<token>` and ALWAYS exit 0.
- Data source: `${LEADV2_CLAUDE_BURN_DIR:-$HOME/.claude/burn}/history.db`, table `hourly`
  (columns: hour_key TEXT 'YYYY-MM-DDTHH' UTC, cc_sum, cr_sum, input_sum, output_sum).
  burn24h = `SELECT COALESCE(SUM(cc_sum+cr_sum+input_sum+output_sum),0) FROM hourly WHERE hour_key >= <utc now - 24h, same format>`.
- Knobs (env): `LEADV2_BURN_GOVERNOR` default 1 (0 → verdict=ok reason=disabled);
  `LEADV2_BURN_SOFT_24H` default 800000000; `LEADV2_BURN_HARD_24H` default 1300000000.
- Fail-open: sqlite3 missing, db missing, table missing, query error → `verdict=ok reason=no_telemetry`.
  Never exit non-zero, never block work on missing telemetry.
- Validate: hard must be > soft; if misconfigured, treat as defaults and say reason=bad_config
  on an ok verdict only when disabled — otherwise just use defaults.
- bash, `set -euo pipefail` with the same ERR-trap fail-open style the other hooks use.

### 2. `scripts/leadv2-dispatch-code.sh` — pre-arm burn gate
Insert a gate early in the dispatch path (alongside/near the existing quota checks, BEFORE
worktree creation and worker launch, AFTER usage/arg parsing):
- Call `leadv2-burn-governor.sh verdict` (resolve via SCRIPT_DIR, same pattern as other
  sibling-script calls in this file).
- `hard` → refuse the dispatch: print a loud line
  `[leadv2-dispatch-code] ⛔ BURN GATE: 24h burn <N> >= hard cap <H> — lane refused, task parked`,
  journal it (same journal mechanism the quota path uses, event `burn_gate verdict=hard`),
  park the task into `docs/leadv2/burn-deferred.jsonl` (same row shape as the existing
  glm-deferred park: sig8, mission_path, founder_task_id, refused_at, reason=burn_hard_24h,
  burn24h) and exit with a NEW dedicated exit code 6 (document in usage()). Do NOT reuse the
  glm-deferred file — burn parks are provider-agnostic.
  `LEADV2_BURN_OVERRIDE=1` bypasses the hard refusal (journaled as burn_gate overridden=1).
  `--force` must NOT bypass it.
- `soft` → proceed, but print
  `[leadv2-dispatch-code] ⚠ BURN GATE: 24h burn <N> >= soft <S> — dispatch allowed, consider deferring non-critical lanes`
  and journal `burn_gate verdict=soft`.
- `ok`/fail-open → zero output, zero journal noise.
- Add a `burn-deferred [--list|--retry-all|--json]` subcommand mirroring `glm-deferred`
  semantics (retry re-dispatches rows only when the governor verdict is no longer hard).

### 3. `hooks/leadv2-compact-trigger.sh` — interactive emergency escalation
Today `decision:block → /compact` fires only on the daemon-mode branch. Extend: at the
EMERGENCY level (existing threshold env), INTERACTIVE sessions also get the one-shot
`{"decision":"block","reason":"/compact"}` — reuse the exact same one-shot MARKER-file guard
so it can never loop (stop_hook_active must still short-circuit). Behind
`LEADV2_COMPACT_INTERACTIVE_BLOCK` default 1. WARN and HARD levels stay warn-only.
Read the existing level ladder in the file first and keep its conventions.

### 4. Tests
Add a test file under the plugin's existing tests/ layout (follow the conventions there):
- governor: fixture sqlite db in a temp dir (create table hourly, insert rows) → assert
  ok/soft/hard boundaries and the fail-open cases (no db, no sqlite3 on PATH via PATH stub).
- dispatcher gate: with LEADV2_DISPATCH_SPAWN=0 and a stubbed governor (env-point
  LEADV2_BURN_GOVERNOR_BIN or PATH shim) assert hard → exit 6 + burn-deferred row written,
  soft → exit 0 resolve-only proceeds.

### 5. Docs
- usage() in the dispatcher: new exit code 6 + burn-deferred subcommand + env knobs.
- One short section in the plugin docs where dispatcher env knobs are documented (grep for
  where LEADV2_DISPATCH_ENFORCE is documented and put it beside).

## Constraints
- Never edit anything outside ~/Projects/leadv2.
- Fail-open everywhere: the governor must never be able to brick dispatch when telemetry is
  absent (fresh machine, db moved).
- Keep the dispatcher diff minimal and localized — this file has 324 recorded bug fixes;
  do not refactor surrounding code.
- Match existing shell style (ERR traps, LEADV2_HOOK_PROFILE pattern where applicable).

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-b22dc98b" "<question>" \
