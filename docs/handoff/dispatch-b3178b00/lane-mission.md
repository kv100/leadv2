Product implementation task dispatch-b3178b00. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# PLUGIN-CORE-OFFLINE-4RED-01 — ROUND 2 FINISHER (resume of lane e2e9d9b2)

## Outcome (one)
Close the round-1 review FAIL: fix CRITICAL-1, HIGH-1, MEDIUM-1, MEDIUM-2 from
docs/handoff/dispatch-e2e9d9b2-review/critic.full.md (read it FIRST — file:line + falsification
evidence + prescribed fixes), then COMMIT clean work on the lane branch.

## Context
Resuming lane worktree .claude/worktrees/e2e9d9b2 with your round-1 diff in the tree
(uncommitted). Round-1 verdict FAIL recorded (diff fb29baff, reviewer=opus-critic).
Your "50/0" was order-dependent: the critic's full-runner run got 46/4 with a DIFFERENT red set.

## Required fixes
1. CRITICAL-1 hermetic determinism: kill the order-dependence. fixtures/pe_run_cache_sync.sh:30
   file-level DRY_RUN default is the wrong lever — pass mode explicitly per call site
   (--dry-run/--write, as test-lane-truth-batch-01.sh does). Root-cause WHY the runner order
   changes outcomes (shared state between suites: cache dirs, ledgers, env leakage) and isolate it.
   Acceptance: run-core-offline.sh 50/0 TWICE BACK-TO-BACK, raw output of both runs pasted.
2. HIGH-1: test-fg-dispatch-guard.sh must assert the MANIFEST registration entry (or a
   LEADV2_DISPATCH_TRACE=1 trace naming the guard), not a grep that a comment satisfies.
   Falsify: unregistered-manifest mutant goes RED.
3. MEDIUM-1 hermetic writes: no test writes tracked repo files. Journal root under TMP_ROOT.
   REMOVE the +90 journal lines (docs/leadv2/tasks/dispatch-567ba028/, dispatch-59ae8b51/) from
   the diff before committing; runner must leave `git status` clean on
   docs/leadv2/{active.yaml,bus.jsonl,merge-queue.jsonl,open-threads.md,questions}.
4. MEDIUM-2: the Codex full-cycle suite must not read live quota state — stub the quota/probe
   layer so it is deterministic offline.

## Non-goals
Same as round 1: no assertion weakening; no engine-code changes unless a suite exposes a real
regression (cite it); test-lane-writes-scoping C3/L12 out of scope; do not touch files owned by
other live lanes (leadv2-dispatch-product-close.sh core logic, leadv2-review-run.sh mission text).

## Acceptance
- run-core-offline.sh: 50/0 twice back-to-back (raw output both times).
- HIGH-1 mutant falsification shown red.
- `git status` clean outside your write-set after a full runner pass.
- bash -n + shellcheck clean; commit on lane branch.

## Rollback
Single revert of lane commit(s).

## Terminal artifact
Commit sha + per-finding fix note + both raw 50/0 outputs + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-b3178b00" "<question>" \
