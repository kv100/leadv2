Product implementation task dispatch-b413968c. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# QUOTA-GATE-PARITY-01 — enforce declared ceilings for ALL three providers, not just GLM

Repo: ~/Projects/leadv2. Grounding: `~/Projects/persona-engine/docs/research/three-provider-efficiency-20260824.md`
(gaps #3, #6, #8 — the "build now" set). Read it first.

Current state (verified by the audit): declared quota_ceilings (glm 80/90, codex 90/95,
claude 95/95; review gets the higher ceiling) are enforced ONLY for GLM
(`leadv2-glm-quota-gate.sh`). Codex just ran to 100%/0-remaining with 4 days left in its window
and NOTHING gated it or rerouted its mandatory review duty. Data for all three providers already
sits in `~/.claude/state/leadv2/quota-cache/{glm,codex,anthropic}.json` (refreshed by
leadv2-quota-live.sh with per-bucket TTLs).

## Deliverables

1. [gap #3] Generic provider quota gate: either extend the existing GLM gate into
   `leadv2-provider-quota-gate.sh <provider> <purpose>` (purpose: build|review, picks the
   lower/higher ceiling) reading quota-cache, or add codex/claude gates mirroring the GLM one —
   follow whichever shape the existing gate + its callers make cheapest. Fail-open on missing/
   stale cache (never block on absent telemetry; stale = older than 2x the bucket TTL → treat
   unknown, allow, WARN). Ceilings from the existing config source (find where 80/90/95 live —
   memory says the ladder is currently hardcoded; if hardcoded, centralize into one sourced file
   but do NOT change the values).
2. [gap #6] `leadv2-burn-governor.sh --provider <glm|codex|claude>`: for glm/codex the verdict
   comes from quota-cache remaining_pct vs the ceilings (soft=ceiling-10, hard=ceiling) instead
   of the token DB; default (no flag) stays exactly today's Claude-token-burn behavior
   (backward compatible — existing callers unchanged).
3. [gap #8] Codex-dead reroute: in the dispatch/review path where mandatory Codex review is
   resolved, catch `limit_reached=true` / remaining 0 from codex quota-cache and reroute the
   review to the next arm (sonnet/claude per existing review-pool logic) with a loud journal
   line `codex_dead_reroute` — today a dead Codex silently yields status=unreviewed
   (all_arms_unavailable). Find the exact resolve point (leadv2-review-run.sh
   resolve_review_pool_call and/or dispatcher review pool) and make the reroute happen there;
   do not invent a new pool mechanism.
4. Tests (existing conventions): gate verdicts per provider from fixture cache JSONs incl.
   fail-open cases; governor --provider mapping; the codex-dead reroute (fixture cache with
   limit_reached=true → review resolves to fallback arm, journal line emitted).
5. Docs: one section beside the existing quota/routing docs.

Constraints: minimal localized diffs; never edit outside ~/Projects/leadv2; bash -n + run the
new tests + any touched existing suites. NOTE for planning: gaps #1/#2/#4/#7 (pacing, per-day
budgets, reserve wiring, time-of-week shifting) are explicitly OUT OF SCOPE — they wait for the
Pro-5x tier decision.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-b413968c" "<question>" \
