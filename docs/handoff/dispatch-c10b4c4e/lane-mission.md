Product implementation task dispatch-c10b4c4e. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# SWEEPER-LANE-SAFETY-01 — continuation (worker hit turn cap with work committed)

Repo: ~/Projects/leadv2, lane worktree .claude/worktrees/53b2ae6c — continue on the existing
branch; your commit cc938f4 (lane-protection gate, lib/leadv2-worktree-protected.sh, both
sweepers) is there. Finish the ORIGINAL mission's remaining deliverables (read your own
cc938f4 diff first, then the mission below):

1. Whatever of the original deliverables is not yet done — most likely: tests (fixture repo:
   protected states not swept; truly orphaned swept; unreadable active.yaml sweeps nothing),
   the journal line `worktree_swept id=<id> reason=<...>` on every actual sweep, and the audit
   note for leadv2-orphan-monitor-sweep.sh.
2. bash -n + suites green; commit on the lane branch.

NOTE for your summary: local main meanwhile received an independent narrow fix 2e9b9ff
("ahead=0 newborn lanes not swept") in the SAME hook — do NOT rebase/merge it yourself; just
avoid assuming your worktree text is the only writer. The lead will resolve the merge.

Original mission follows for reference:
# SWEEPER-LANE-SAFETY-01 — the worktree sweeper must never eat a registered lane

Repo: ~/Projects/leadv2. Founder order 2026-08-24 («свипер починить на уровне плагина») after
TWO same-day incidents: lane worktrees b413968c and 43ae4318 were swept mid-lifecycle (one was
gutted to a single docs/ dir, one deleted entirely), each costing a dead worker attempt and a
manual repair/redispatch. Standing memory says the same killed review subsessions before
(feedback-remove-scratch-worktree-before-review).

Suspect: `plugins/leadv2/hooks/leadv2-merged-worktree-sweep.sh` (and audit
`leadv2-orphan-monitor-sweep.sh` for the same class). Its current criterion ("no unmerged
commits") is wrong for lanes: a lane worktree legitimately sits at base (zero unmerged commits)
BEFORE its worker commits anything, and between fix rounds after a merge.

## Deliverables
1. In the sweep candidate check, a worktree is UNTOUCHABLE if ANY of:
   - its id appears in docs/leadv2/active.yaml (any state) of the repo the sweep runs in;
   - a handoff dir docs/handoff/dispatch-<id>/ exists with an `arm-registered` marker and NO
     terminal state recorded (read how product_close stamps terminal — reuse that signal);
   - the worktree has a live registered PID/attempt (reuse the existing liveness helper the
     dispatcher uses, if importable cheaply);
   - a grace window: worktree younger than N hours (default 48, env LEADV2_SWEEP_MIN_AGE_H).
   Only when NONE hold AND no unmerged commits may it sweep. Fail-CLOSED: any read error on
   active.yaml/handoff → skip the sweep for that worktree and log one line.
2. Every actual sweep must journal `worktree_swept id=<id> reason=<...>` (today it is silent —
   both incidents were diagnosed forensically). Every SKIP for a protected lane logs at debug
   level only (no per-turn noise).
3. Tests: fixture repo with worktrees in each protected state → not swept; truly orphaned
   (no active.yaml row, no handoff, old, merged) → swept; active.yaml unreadable → nothing
   swept. Follow existing tests/ conventions.
4. Audit `leadv2-orphan-monitor-sweep.sh` for the same kill-class; fix if same bug, else state
   why it is safe in the summary.
5. Docs: short paragraph in the hooks docs + note in single-lead-mode.md worktree section if
   one exists.

Constraints: minimal diff; never edit outside ~/Projects/leadv2; bash -n + tests green.

## Re-dispatch note (2026-08-24, lead)
Prior lane 18ab6b98 accumulated poisoned dispatcher state (4 worker deaths at SessionStart,
stale terminal markers); this sig-changing note forces a FRESH lane id. Content unchanged.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-c10b4c4e" "<question>" \
