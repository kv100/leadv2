Product implementation task dispatch-c3388a7f. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
<!-- leadv2-prepass base_head=65664adfceba0f798bb294a1eb5229ee728c7ac4 generated_at=2026-08-25T15:30:00Z -->
...
# CTX-COST-GUARDS-01 — close the Bash-side diff-read bypass (plugin-level, all repos)

Work in the PLUGIN repo `~/Projects/leadv2` (`plugins/leadv2/`), never in a consuming repo.
One new sub-guard inside the EXISTING dispatcher — do not create a new hook file, do not create a
copy of anything in persona-engine.

## Premise, already verified by the lead — do not re-derive

- `plugins/leadv2/hooks/leadv2-read-gate.sh` (registered in `hooks.json`, PreToolUse:**Read**)
  already merges force-read-limit + read-dedup-hard + lead-read-guard. The Read side is covered.
- `leadv2-bash-output-cap.sh` is **PostToolUse** — it warns after the tokens are already paid.
- `leadv2-bash-pre-dispatch.sh` (PreToolUse:Bash) currently has exactly two sub-guards:
  `leadv2-block-bash-heredoc` and `leadv2-block-fg-dispatch`. **Nothing guards diff reading.**
- Measured (persona-engine transcripts, 2026-08-25): the heaviest Bash calls in the whole history
  are `sed -n '1,340p' …/review.diff` (7,393 tok), `sed -n '1,220p' …/build-attempt-1.diff`
  (7,407), `sed -n '1,200p' …/review.diff` — the last one **twice**. This is the Read gate's
  bypass: the same artifact, fetched through Bash.

## The change — sub-guard `leadv2-warn-bash-diff-read`

Add it to `plugins/leadv2/hooks/leadv2-bash-pre-dispatch.sh`, following the exact shape of the two
existing sub-guards (same emit format, same env-var naming, same fail-open trap).

Fires when the command reads a diff-shaped artifact into context:
- the command contains a path matching `\.diff$` or `\.patch$`, **or** `git diff` / `git show`
  without `--stat` / `--name-only` / `--numstat`, AND
- it is an unbounded read (`cat`, `less`, `awk`, `grep` with no line cap; `git diff` with no
  `--stat`), or a bounded one whose window exceeds `LEADV2_DIFF_READ_MAX_LINES` (default 200) —
  `sed -n 'A,Bp'`, `head -N`, `tail -N`.

Behaviour:
- **Advisory WARN, exit 0** by default. `LEADV2_DIFF_READ_DENY=1` upgrades to a block (exit 2,
  same text). `LEADV2_DIFF_READ_GUARD=0` disables. Default MUST be warn — a block here would hit
  reviewer workers doing their job.
- Skip entirely when the caller is a subagent/worker, not the lead. `leadv2-lead-read-guard.sh`
  detects this from the input's `agent_type` field — read that file and reuse the same predicate.
- The message must be 3 short lines naming the cheaper path: `git diff --stat`, then the review
  engine's `review-findings.json` in the same handoff dir, then `repowise distill <cmd>`
  (`repowise expand <ref>` recovers what was elided — never re-run the command).

## Hard requirements

1. **Fail-open on every error path** — bad JSON, missing `jq`, unreadable stdin all exit 0.
2. No network, no git invocation that can block; keep the added path under ~15 ms.
3. Do not touch `hooks.json` (the dispatcher is already registered) and do not change the
   behaviour of the two existing sub-guards.
4. The regex must not fire on `--stat` / `--name-only` / `--numstat`, nor on `ls`, `wc -l`,
   `grep -c`, nor on a `git diff --stat` pipeline.

## Verification you MUST run and paste

Pipe real PreToolUse JSON into the hook and show a red/green table:

| input | expected |
|---|---|
| `sed -n '1,340p' docs/handoff/x/review.diff` | WARNS |
| `cat /tmp/r4-code.diff` | WARNS |
| `git diff` (no flags) | WARNS |
| `git diff --stat` | silent |
| `sed -n '1,40p' docs/handoff/x/review.diff` | silent (under 200) |
| `wc -l x.diff` / `grep -c foo x.diff` | silent |
| same call with `agent_type` = a subagent | silent |
| `echo 'not json' \| …` | `rc=0`, no output |

Also run `bash -n plugins/leadv2/hooks/leadv2-bash-pre-dispatch.sh`, and re-run one existing
heredoc-guard case to prove no regression.

Deliverable: `docs/handoff/CTX-COST-GUARDS-01/developer.full.md` — the diff, the table with real
output, and one line on anything you found already covered. Last line exactly
`DELIVERABLE_COMPLETE`.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-c3388a7f" "<question>" \
