Product implementation task dispatch-c8474002. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CODEX-LEAD-PILOT-PREP-01 — prepare a measured 1-day pilot: Codex(GPT-5.6) as lead, Claude+GLM workers

Repo: ~/Projects/leadv2 (docs + one config artifact; NO changes to dispatch scripts).
Context docs (read first, bounded): ~/Projects/persona-engine/docs/leadv2/single-lead-mode.md,
docs/research/reverse-bridges-deep-20260824.md §implications,
docs/research/harness-subscription-matrix-20260824.md CORRECTION section,
plugins/leadv2/hooks/hooks.json (enumerate what the hooks enforce — the pilot's Codex lead has
no blocking hooks, so the brief must carry the top gates as prose).

Founder decision: pilot "Codex as lead" — a Codex CLI session (GPT-5.6) orchestrating the
EXISTING leadv2 bash machinery as tools: leadv2-dispatch-code.sh for GLM/Sonnet worker lanes
(claude binary under its own subscription — legal), review via leadv2-review-run.sh, journals,
status. All three subscriptions stay in their sanctioned clients; only the orchestrator brain
moves.

## Deliverables
1. `plugins/leadv2/docs/codex-lead-pilot-runbook.md`:
   - Launch: exact codex CLI invocation (model, effort, cwd persona-engine, AGENTS.md pickup),
     session lifetime rules (one working block, no marathon).
   - Task menu: 2-4 real Standard-class tasks from docs/tasks.yaml appropriate for a pilot
     (pick criteria, not specific ids).
   - Measurement protocol: before/after snapshots of ~/.claude/state/leadv2/quota-cache/*.json,
     burn digest 24h delta, lanes landed vs stalled vs parked, founder interventions count,
     and a "gate violations" checklist — things the Claude-side blocking hooks would have
     stopped (opus-spawn, shared-tree edits, heredocs, direct codex exec, worktree scope) that
     the Codex lead must self-police; reviewer checks the transcript after.
   - Pre-registered success/fail criteria (numbers, not vibes) + rollback (just stop using the
     codex session; nothing persistent changes).
2. `AGENTS.md` addition for the PILOT (write as `plugins/leadv2/docs/codex-lead-AGENTS-pilot.md`
   — a file the founder copies to the repo root as AGENTS.md for the pilot day, NOT auto-
   installed): distill the lead protocol to <=150 lines: single-lead WIP rules, dispatch door
   (leadv2-dispatch-code.sh usage + exit codes incl. 6=burn-park), mandatory review gate, the
   top-10 hard rules from hooks as prose (no git reset/clean in shared trees, no rm -rf, ledger
   discipline, journal appends, Russian founder chat, verify-by-evidence), and where state
   lives (active.yaml, handoff dirs, founder-status).
3. NO live pilot run in this lane — prep only. The founder starts the pilot himself.

Constraints: docs only + the one template file; never touch dispatch scripts or hooks; bash -n
n/a; have the critic-facing summary state what was deliberately left out of the 150-line brief.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-c8474002" "<question>" \
