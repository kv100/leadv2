Product implementation task dispatch-cbe25ccc. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BROAD-STATUS-RELAY-SCOPE-01 — ROUND 2 FINISHER (resume of lane 91f975bf)

## Outcome (one)
Close the round-1 review FAIL: fix the 3 HIGHs + 5 MEDIUMs from the review body at
docs/handoff/dispatch-91f975bf-review-critic.full.md (full report with executed scenarios; read
FIRST). Commit on the lane branch.

Punch list:
1. HIGH (leadv2-beat-owner.sh:33-49): owner freshness must also require the recorded session to be
   ALIVE, not just an epoch — per-session heartbeat file .pulse-session.<sid>, required < 1×BEAT_S,
   else unresolved (fail-open to full relay). Today a fresh owner file naming a dead session makes
   EVERY live session a guest for 3600s — and this FIRES ON SHIP because the mandated restart mints
   a new session_id (reviewer Scenario E).
2. HIGH (leadv2-pulse-beat.sh +115-124): ownership must NOT be "whoever's --check won the throttle
   race" — only a session with >=1 LIVE LANE may claim ownership (the .supervise-active rows are a
   dead path: supervisor retired). Otherwise the original incident (focused session gets the
   25-row dump) is not fixed.
3. HIGH (tests): the suite must drive the REAL mechanism, not the LEADV2_BEAT_OWNER_OVERRIDE seam:
   create real .pulse-beat-owner state; cover _lv2_is_ancestor, _lv2_beat_owner_fresh (alive vs
   dead session), and the LEADV2_BEAT_OWNER_SESSION handoff. Red-first each case.
4. MEDIUMs: register the suite in run-core-offline.sh; chmod files properly; remove the
   unconditional "paste verbatim" inject in leadv2-supervisor-mode-reinject.sh:140 (contradiction);
   reconcile the guest line wording with the anchor ("relay only that single line"); bound the
   `find … -delete` on PostToolUse.

## Off_limits
Same write-set as round 1 (hooks + pulse-beat + resolver + tests). No product-close, no review-run.
Deliverable must repeat loudly: changed hooks (leadv2-single-lead-beat.sh, leadv2-task-anchor.sh +
any new) need CACHE COPY + SESSION RESTART to go live.

## Acceptance
- Reviewer's Scenario E now yields unresolved→full relay (raw output); ownership claimed only by a
  live-lane session (raw outputs of the three contexts re-run with real state, no seam).
- Suite registered and green in run-core-offline.sh; red-first proofs; bash -n + shellcheck; commit.

## Rollback
Single revert.

## Terminal artifact
Commit sha + per-finding note + raw scenario outputs + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-cbe25ccc" "<question>" \
