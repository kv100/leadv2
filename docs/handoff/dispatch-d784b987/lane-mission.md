Product implementation task dispatch-d784b987. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# CLAIM-EVIDENCE-GATE-01 — factual claims carry evidence or an UNVERIFIED tag (SHARED plugin, founder-approved 2026-08-19)

## Outcome (one)
Two levers, both in ~/Projects/leadv2 (applies to all repos):
1. **Deliverable contract**: the subagent operating rules (plugins/leadv2/skills/leadv2-subagent-protocol/
   or wherever the live subagent preamble text actually lives — TRACE the live reader first, do not
   edit a dead template) gain a hard rule: every factual claim about an external system/API
   (endpoint behavior, rate limits, auth flow, schema, provider quirk) must be immediately followed
   by its probe artifact (curl/log excerpt/doc link + live check output) OR explicitly tagged
   `UNVERIFIED:`. An untagged evidence-free claim is a protocol violation.
2. **Review lens**: the round-1 exhaustive review mission text in
   plugins/leadv2/scripts/leadv2-review-run.sh (landed today, REVIEW-ROUND1-EXHAUSTIVE-01 — build on
   top of current main, commit 6b79c2c+) gains a fifth lens: "claims-without-evidence — enumerate
   every factual claim about an external system in the diff/deliverable; each must have inline
   evidence or an UNVERIFIED tag; an untagged unverified claim that DRIVES a decision (code path,
   config value, limit) is a BLOCKING finding; a tagged one is a MEDIUM at most."

## Premise
Founder observation 2026-08-19 (get-many-follow-bot, m3-market): unverified API beliefs become
truth because the lead trusts worker conclusions. Standing evidence: 5/5 worker claims about code
died when a reviewer executed them (feedback_worker_claims_about_code_are_unreliable).

## Write set
- plugins/leadv2/scripts/leadv2-review-run.sh (mission text ONLY — do not touch round detection /
  arm selection / pool logic landed today)
- the subagent-protocol skill file (trace it: plugins/leadv2/skills/leadv2-subagent-protocol/SKILL.md
  or equivalent live preamble source — cite how you proved it is the live reader)
- plugins/leadv2/scripts/tests/ (extend test-review-round-exhaustive.sh or add a suite; register
  new suite in run-core-offline.sh SUITE_DEFS)

## Non-goals / off_limits
- leadv2-dispatch-product-close.sh (T2 lane owns it).
- No new env vars, no gate machinery changes — text contracts + tests only.

## Acceptance
- Generated round-1 mission contains the claims-without-evidence lens (paste sample).
- Subagent preamble contains the UNVERIFIED contract (paste the live-reader proof + the text).
- Tests red-first against pinned pre-fix baseline (content-probe pattern per
  test-review-gate-scope-evidence.sh; GREEN_PRE_FIX>0 → non-zero exit).
- run-core-offline.sh green forward AND with LEADV2_CORE_OFFLINE_REVERSE=1; bash -n + shellcheck.
...
## Terminal artifact
Commit sha + both text samples + raw red/green + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-d784b987" "<question>" \
