Product implementation task dispatch-dd95798f. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# BUILDER-SELFCHECK-GATE-01 — ROUND 2 FINISHER (resume of lane 9c027877)

Round-1 verdict FAIL (diff=1493fbdf, reviewer=opus-critic). Fix C1-C3, H1-H3, M1-M4. Findings
verbatim (reviewer executed everything):

C1 RECURSION: lib/leadv2-builder-selfcheck.sh:180-191 runs tests/run-all.sh --scope changed,
which reaches run-core-offline.sh → suites that spawn leadv2-dispatch-product-close.sh → selfcheck
→ run-all.sh… (bounded only by the 900s timeout; proven in e2e-gate.log:844-880, every fixture
dies failed=suites:run-all:timeout). Fix: export LEADV2_BUILDER_SELFCHECK=0 + a depth guard before
invoking any suite, and never call the repo-level runner from a gate the runner itself reaches.
C2 lib:32-44 no-gtimeout fallback: watcher subshell inherits stdout of a command substitution
(product-close.sh:1834), kill leaves the sleep child holding the pipe → caller hangs the FULL
timeout (measured 30s on a /bin/echo). Fix: ( … ) >/dev/null 2>&1 & and kill the process group.
C3 three suites diff-caused red (review body persist 2/8-fail, product-close waits-for-worker 6/7,
report-only gate C4) — all green with flag=0. Root-cause each; the gate must not alter those paths.
H1 no baseline comparison: inherited main-red suites reported as selfcheck_failed against the
builder. Restrict the suite check to syntax + diff-scoped suites vs merge-base (repo harness style).
H2 register tests/test-builder-selfcheck-gate.sh in run-core-offline.sh (and trim its 6-min wall).
H3 cover the suite-runner branch (delegate path, always/never, timeout wrapper, stem fallback) —
current 5 cases all SKIP(no_matching_suite); exactly where C1/C2 live. Red-first each.
M1 checks:0 must be DEGRADED/rc2, never GREEN; emit checks=/skipped= fields.
M2 use -f (not -x) for tests/run-all.sh detection (persona-engine's is 644).
M3 resolve suites from ${diff_root}/tests, never the plugin tree.
M4 restore the 2 out-of-write-set journal.md files.

## Acceptance
- The reviewer's recursion probe: a lane in the leadv2 repo completes the selfcheck without
  invoking product-close recursively (show the depth-guard journal line).
- The 3 diff-caused suites green WITH flag=1 (raw before/after).
- Gate-ordering probe still holds (bash -n fail → selfcheck_failed, no review arm).
- New/updated tests red-first; suite registered; run-core-offline delta only your suite;
  bash -n + shellcheck; COMMIT on lane branch.

## Off_limits
leadv2-review-run.sh; everything outside round-1 write-set.
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-dd95798f" "<question>" \
