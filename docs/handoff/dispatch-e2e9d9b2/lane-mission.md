Product implementation task dispatch-e2e9d9b2. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# PLUGIN-CORE-OFFLINE-4RED-01 — make core-offline green on untouched main

## Outcome (one)
`bash plugins/leadv2/scripts/tests/run-core-offline.sh` on canonical main prints
`suites passed=50 failed=0` (currently 46/4). The 4 red suites:
1. dispatch refusal fallback chain
2. plugin sync quarantine/dry-run safety
3. foreground-dispatch guard hook
4. lane truth batch (log_path + quarantine convergence)

## Why (premise, re-proven 2026-08-19)
These 4 are red on UNTOUCHED main (verified twice: canonical run + lane run, identical 46/4).
The e2e gate treats them as regressions and killed two consecutive lanes of
REVIEW-GATE-INFRA-01 with a false `cause=e2e_regression`. Every future plugin lane dies the
same way until main is green. Evidence: docs/handoff/dispatch-049e0e9e/e2e-gate.log and
docs/handoff/dispatch-4c118ab6/e2e-gate.log (identical failure set), journal note in
docs/leadv2/tasks/dispatch-049e0e9e/journal.md ("verdict FALSE").

## Method (per suite — diagnose before touching anything)
For each red suite decide with evidence which of these it is:
(a) the TEST drifted (asserts an old contract; code is right) → fix the test to the current
    contract, cite the commit that changed the contract;
(b) the CODE regressed (test is right) → fix the code;
(c) env-coupled test (reads live quota/lockout state, network, wall clock) → isolate it
    (stub/fixture), because "offline" suites must be deterministic. NOTE: round-1 observations
    suggest at least one suite behaves differently depending on live GLM quota-lockout state
    (a real `glm_refused_quota_gate,sonnet_failed_launcher,codex_dead_no_first_byte` line
    appeared inside a test run) — suspect (c) for "dispatch refusal fallback chain".
NEVER weaken an assertion to green a suite; every fix keeps the suite falsifiable.

## Write set
- plugins/leadv2/scripts/tests/** (suite fixes/isolation)
- plugins/leadv2/scripts/** and plugins/leadv2/hooks/** ONLY where a suite exposes a real code
  regression (cite it). If you fix a hook file, SAY SO in the deliverable — hooks need a
  cache copy + session restart to go live.

## Non-goals
- test-lane-writes-scoping C3/L12 (separate known pre-existing, out of scope).
- No new features; no gate-policy changes; no touching leadv2-dispatch-product-close.sh
  logic landed today (commits 019ec29/85ae886) except test-only interactions.

## Acceptance
- run-core-offline.sh on the lane tree: passed=50 failed=0, run TWICE back-to-back (flake check).
- Each formerly-red suite: raw output before (red) and after (green) + one-line root cause
  (a/b/c classification).
- bash -n + shellcheck clean on touched files.

## Rollback
Single revert of lane commit(s).

## Terminal artifact
Commit(s) + per-suite root-cause table + raw red/green output + DELIVERABLE_COMPLETE.

## Dispatch note
Retry 2 (2026-08-19): first dispatch parked — architect prepass hit the 420s timeout twice while live-probing the suites; timeout raised for this retry.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-e2e9d9b2" "<question>" \
