Product implementation task dispatch-e8c1289f. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# REVIEW-ROUNDCAP-01 — enforce the declared max-2-review-rounds policy (autoresearch F4)

Repo: ~/Projects/leadv2. Evidence: `~/Projects/persona-engine/docs/research/token-burn-anatomy-20260823.md`
(43% of critic sessions on 2026-08-23 were re-review rounds on the same dispatch id; one PARKED
lane re-reviewed 9×, each paying full ~110-140K base) and autoresearch finding F4 in
`~/Projects/persona-engine/.claude/autoresearch/sessions/token-economy-2026-08-23/findings.md`.

Gap (verified by the F4 finding): `.claude/scripts/leadv2-review-run.sh` (canonical:
`plugins/leadv2/scripts/` — check which is the one-inode canonical and edit THERE) has diff-hash
exact dedup only; NO round counter exists anywhere (`grep -rn "round" the file` → zero relevant
hits), even though the leadv2-review workflow doc states "max 2 rounds then architect escape".
Any tiny fix changes the hash, so stuck lanes re-review forever.

## Deliverables
1. Round counter keyed on the task/dispatch id, persisted beside the existing diff-hash record
   store (same directory/mechanism — read how record-review persists, mirror it). Increment on
   each non-dedup review verdict for that task.
2. At round > LEADV2_REVIEW_MAX_ROUNDS (default 2): do NOT run another full review spawn.
   Instead emit a loud terminal line + journal event `review_roundcap task=<id> rounds=<n>`
   and write a `review-roundcap-escalation.md` into the lane handoff dir stating the lane needs
   architect escalation or PARK — matching the declared policy. Env override
   LEADV2_REVIEW_MAX_ROUNDS=N; 0 = unlimited (kill-switch).
3. Fail-open: any error in the counter machinery must never block a legitimate review (fall
   through to running the review).
4. Tests in the existing tests/ convention: round increments, cap refusal at 3rd round, dedup
   rounds don't increment, kill-switch, fail-open on corrupt counter file.
5. Docs: one paragraph beside the review engine docs.

Constraints: minimal localized diff; never edit outside ~/Projects/leadv2; bash -n + run the new
test + any existing review-engine tests you touch.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-e8c1289f" "<question>" \
