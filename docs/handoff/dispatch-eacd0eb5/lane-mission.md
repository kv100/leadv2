Product implementation task dispatch-eacd0eb5. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# SUPERVISOR-DELETE-01 — delete the supervisor machinery entirely (founder order 2026-08-19)

«мы его выключили и никогда не включим, его вообще надо удалить полностью». Supersedes
keep-as-dead-code. Single-lead mode is permanent (2026-08-17 order: anything reachable only from
the supervise loop is dead code — migrate what the live path still needs, delete the rest).

## Phase 1 — live-path audit (do FIRST, cite grep output)
Enumerate every reader of the supervise family from the LIVE single-lead path:
- grep -rn "leadv2-supervise" across plugins/leadv2/{scripts,hooks,skills,docs} and the repo-level
  .claude/ trees; classify each hit: (a) supervise-only (dies with it), (b) live-path dependency
  (MIGRATE first — the PULSE-IN-SINGLE-LEAD-01 precedent: the beat lived in the loop and had to
  move out), (c) doc/archive mention (update or leave).
- Known live-path suspects to check explicitly: leadv2-single-lead-beat.sh (already migrated),
  active-registry reconciliation (leadv2-supervise.sh D-d adoption/prune — is anything on the
  live path calling it? backlog-pump? phase-backfill?), leadv2-supervise-watchdog.sh.
- Anything MIGRATED gets its own commit before the delete commit.

## Phase 2 — delete
- plugins/leadv2/scripts/leadv2-supervise{,-loop,-pick,-resume,-watchdog}.sh
- plugins/leadv2/scripts/tests/test-supervise-v2.sh + its SUITE_DEFS row in run-core-offline.sh
  ("supervisor reconciliation")
- supervisor-role docs (plugins/leadv2/docs/supervisor-role.md) + supervisor-only hook/skill refs
- /leadv2 supervise entry in the skill (replace with a one-line "retired, founder order
  2026-08-19" stub that refuses)

## Off_limits
- leadv2-dispatch-product-close.sh (T2 lane owns it)
- worker-side turncap checkpoint machinery
- Do NOT delete history/archive docs; docs/handoff/** untouched.

## Acceptance
- grep -rlE "leadv2-supervise(-loop|-pick|-resume|-watchdog)?\.sh" over plugins/leadv2/{scripts,hooks,skills}
  returns ZERO hits outside tests-of-deleted (which are themselves deleted).
- run-core-offline.sh forward AND LEADV2_CORE_OFFLINE_REVERSE=1 green (suite count drops by 1).
- bash -n + shellcheck -S warning on every edited (not deleted) script.
- A dispatch smoke: bash plugins/leadv2/scripts/tests/test-lane-placement-pin.sh green (proves
  dispatch path unaffected).
- COMMIT(s) on lane branch: migrate-commits (if any) then one delete commit.

## Terminal artifact
Commit shas + the phase-1 classification table + raw greps + suite outputs + DELIVERABLE_COMPLETE.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-eacd0eb5" "<question>" \
