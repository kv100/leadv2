Product implementation task dispatch-eb2d7143. Implement ONLY the scoped design below; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
# V3-GLM-LADDER-01 — deferred-GLM queue + Codex credit watchdog + loud sonnet exceptions

## Premise (incident 2026-08-19, this session)
GLM-FIRST-01 was silently inverted for a full day: every dispatch logged
`arm_refused by=router model=glm reason=glm_refused_quota_gate`, Codex showed
`credits={"balance":"0","has_credits":false}`, and 100% of workers ran on sonnet. Nobody was told.
The founder learned it from the lead reading raw dispatcher logs.

## Outcome (three levers, plugin repo ~/Projects/leadv2)
1. **Deferred-GLM queue**: when the router refuses glm on quota for a task whose kind/classification
   is glm-fitting, record the task into a park file (docs/leadv2/glm-deferred.jsonl: sig8, mission
   path, refused_at, quota_pct) BEFORE falling through to sonnet. A dispatcher subcommand
   `glm-deferred [--list|--retry-all]` re-dispatches parked rows; the quota gate check answers
   whether the window reopened.
2. **Codex credit watchdog**: at every route_headroom computation where codex has_credits=false,
   emit ONE deduped journal line per 24h (`codex_credits_empty since=<ts>`) and surface it in the
   beat/founder-status facts (repo_facts), so an empty balance is visible in the pulse, not buried.
3. **Loud sonnet exceptions**: every arm choice that lands on sonnet AFTER a glm refusal appends a
   one-line entry to a daily counter file the broad-status renderer reads (e.g.
   docs/leadv2/.arm-exceptions-YYYYMMDD: count + last reason). The founder-status queue section
   shows "sonnet-фолбэков сегодня: N (glm quota)" when N>0.

## Constraints
- Do NOT change routing order or ceilings (quota ladder is intentionally hardcoded —
  feedback_quota_ceilings_per_provider). This task makes degradation VISIBLE and RECOVERABLE,
  not different.
- No new env vars. Small, focused diffs in leadv2-dispatch-code.sh routing block +
  leadv2-broad-status.sh facts collection + (new) glm-deferred subcommand.
- Off_limits: leadv2-dispatch-product-close.sh; supervise* files (deletion lane owns them);
  run-core-offline.sh SUITE_DEFS row addition is allowed (one line) but nothing else in it.

## Tests (red-first, content-probe baseline per test-review-gate-scope-evidence.sh pattern)
plugins/leadv2/scripts/tests/test-glm-deferred-ladder.sh: (a) quota-refused glm-fitting task →
park row written before sonnet fallback; (b) glm-deferred --list shows it; (c) credit watchdog
line dedupes within 24h; (d) exception counter increments and renderer picks it up (rendered
probe, not source grep — the CLAIM-EVIDENCE lesson). Register in SUITE_DEFS.

## Acceptance
- Raw red/green for the suite; run-core-offline fwd+rev green in lane; bash -n + shellcheck.
- A dry-run dispatch with a forced quota refusal (env/fixture) shows: park row + sonnet worker +
  exception counter=1 (raw outputs).
...
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-eb2d7143" "<question>" \
