Product implementation task dispatch-f3ae347e. Implement this mechanism-closed design; preserve its non-goals. Before closing, run the required end-to-end gate and the cross-provider review gate recorded for this task.
...
CLAUDE-PROFILE-SELECT-FINISH-01 — finish the multi-Claude-account dispatch selector and land it on main.

Context: commits 8d00999 (wip) + 747c1ad live ONLY on branch worktree-tmux-statusline, not on main. The selector (plugins/leadv2/scripts/leadv2-claude-profile-select.sh + lib/leadv2-claude-profile-pick.py + leadv2-quota-read.py changes + claude-subsession.sh wiring) picks the least-quota-burned of 2+ logged-in Claude accounts. Review found a critical wiring gap.

Tasks:
1. Cherry-pick / merge the selector commits from worktree-tmux-statusline onto a fresh lane branch off main (do NOT bring unrelated uncommitted changes from that branch).
2. Fix the critical gap: `credential_source` (keychain:<service> | file:<path>) is dropped between leadv2-claude-profile-pick.py:78 (pick output line) and claude-subsession.sh:605 (consumer regex). claude-subsession.sh:611 exports only CLAUDE_CONFIG_DIR — the spawned `claude` never learns the winning keychain service. Thread credential_source through the pick line + regex; for keychain:<service> export CLAUDE_CODE_CREDENTIALS_SERVICE=<service> (verify the exact env var a real Claude Code session honors — leadv2-quota-read.py:413 references it) before spawn.
3. Add an integration test asserting the spawned child's credential service matches the winning profile's keychain: source (extend tests/test-claude-profile-select.sh; current I1 only asserts CLAUDE_CONFIG_DIR).
4. External-consumer safety (founder requirement): the feature must stay default-inert for a plugin consumer with no profile registry — confirm the existing fail-open to single_profile covers registry-missing/malformed/<2-entries, and document the feature as internal/advanced opt-in in the docs page.
5. Run tests/test-claude-profile-select.sh + the quota-gate suite green, then land on main.

Bonus (same lane, small): leadv2-codex-quota-gate.sh has a documented-but-untested fail-open when leadv2-provider-quota-gate.sh loses its exec bit (rc 126 → gate silently passes). Add a test asserting rc-126 behavior emits a WARN at minimum.

## LEAD RULING (2026-08-25, resolves the census-falsified stops — binding)
The architect prepass for dispatch-16fbe872 (docs/handoff/dispatch-16fbe872/architect-prepass.md) is ADOPTED as the authoritative design, with one amendment resolving the C3/C4-vs-I5 contradiction that stopped two workers:
- C3 ships in the LEAN self-consistency form (prepass step 2): both registry columns present, service non-empty, config_dir readable; `# lean:` marker at the guard site; cross-account identity verification is DEFERRED (quota-read probe payload carries no identity field — confirmed).
- Test I5 (`reason=cred_mismatch`) is DROPPED — in lean form no cred_mismatch path exists, and inventing one is forbidden. Keep I2/I3/I4/I6 exactly as the prepass defines them. Record the deferral in the deliverable and in the model-routing doc section (C5).
- Everything else: C1, C2, C4 (minus I5), C5, C6 and all Non-goals stand as written in that prepass. My earlier mission text about CLAUDE_CODE_CREDENTIALS_SERVICE and cherry-pick is SUPERSEDED by prepass P1/P2 — do not export that var, do not cherry-pick.
Do not stop on census grounds again: this ruling IS the design authority for the contradiction. Implement, test, commit in the pinned worktree.

---
If you hit a decision you cannot safely make yourself (including destructive
options, a policy conflict, or missing authorization), ask via the async
question channel and wait for the answer rather than guessing or stalling:
  bash "${CLAUDE_PLUGIN_ROOT}/../../scripts/leadv2-ask.sh" "dispatch-f3ae347e" "<question>" \
