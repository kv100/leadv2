...
contains "$PILOT_BRIEF" 'Lifecycle hooks record start/stop evidence' && contains "$PILOT_BRIEF" 'list_agents` supplies live state' && contains "$PILOT_BRIEF" 'pulse audit trail' && contains "$PILOT_BRIEF" 'at most one pulse' && contains "$PILOT_BRIEF" 'per 60 seconds of lead activity' && pass "native profile: installed brief has truthful pulse contract" || fail "native profile: installed brief pulse contract is incomplete"
...
contains "$STATUS_PROMPT" 'Lifecycle hooks record start/stop evidence' && contains "$STATUS_PROMPT" 'pulse audit trail' && contains "$STATUS_PROMPT" 'no idle/tick hook' && contains "$STATUS_PROMPT" 'at most one pulse' && pass "native profile: installed fallback pulse is truthful" || fail "native profile: installed fallback pulse is incomplete"
