...
| `/leadv2 supervise` | **Provider-aware full-cycle supervisor (distinct session mode).** The supervisor owns no task, phase, task lock, worktree, or child prompt history and does not consume a worker slot: (1) reconcile+adopt existing work via `scripts/leadv2-supervise.sh --json` -> (2) `scripts/leadv2-supervise-pick.sh` ranks candidates, founder picks 0-5 -> (3) dispatch each choice through `scripts/leadv2-fanout.sh --provider auto --headless`. Every child is an independent Claude or Codex lead session, claims its own Phase-0 worktree, executes the complete Phase 0..8 lifecycle with the same guards, and is complete only when canonical `phase8-passed.flag` or its validated shared completion receipt exists; child-internal Workflow/Agent calls are phase helpers, not top-level supervised lanes. Only registry/quota/questions/completion receipts are shared. The deterministic router keeps Heavy/Strategic and high-risk work on Claude/Opus and may send routine work to Codex when CLI, skill, and quota headroom are healthy. (4) Attach `scripts/leadv2-supervise-loop.sh --ensure` via `Monitor`. Questions route through `scripts/leadv2-ask.sh`; a corroborated-dead child is never auto-restarted. See `.claude/skills/leadv2-supervise/SKILL.md`. |
| `/leadv2 fanout [--n N] [--provider auto\|claude\|codex] [--backend tmux\|headless]` | Dispatch the same complete Phase 0..8 child sessions and exit — no picker, reconciliation, or supervision loop attached. `auto` uses `config/session-routing.yaml`; all providers share the final-sentinel, active-registry, question-channel, merge-lock, deploy-gate, and live-verification contracts. Concurrency ceiling is `active.yaml meta.hard_limit` (`--force` may exceed policy caps but never bypasses same-task collision protection). |
