...
**PULSE MODE (default ON):** between phases: absolute silence. Gate 1: one line + wait. Async question: one line + options. Phase 8 close: max 3 lines. The anti-silence pulse tick — the `[ПУЛЬС HH:MMZ] …` line the lead's Monitor relays verbatim from the background `scripts/anti-silence-pulse.sh` run armed at session start — is an allowed output. Narration is model-generated prose about its own work; the pulse is a verbatim relay of a plugin-generated artifact — never a `CronCreate` job. ONE-STATUS-MECHANISM-01 (2026-09-13): this is the ONLY status mechanism. A tick that produced nothing still emits a silence line — тишина is itself an event. Every extra sentence = protocol violation.
