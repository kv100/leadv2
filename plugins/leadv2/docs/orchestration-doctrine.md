...
| --- | --- | --- | --- |
| GLM | Any `kind` the capability matrix admits: code, docs, review, plan, audit, safety, at sizes standard/heavy/bulk. `capability: 4` — the same rung as fable and opus — at `cost: 1`. | work 80%, review 90%. | Founder order 2026-09-09 (GLM-DOES-EVERYTHING-01) retires the old "build-only" line, which was also stale on two facts: the live matrix gives glm `protected: true` AND `review: true`, so it was never excluded from protected or review cells. (`plugins/leadv2/config/leadv2-routing.yaml` `router_v2.capability_matrix`) |
