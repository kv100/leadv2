...
- If queue cache stale → `claude-subsession` (`scripts/claude-subsession.sh`)
> **Optimization:** classify inline FIRST (Phase 1 — there is no classify skill), then execute EnterWorktree + MCP warm only if class ≥ Standard. This avoids ~2K tokens of worktree setup for Trivial/Light tasks. If classification returns Trivial/Light → jump directly to Phase 4 Build without worktree.

- **Enter worktree:** `EnterWorktree(name="<task-id>")` — creates `.claude/worktrees/<task-id>` on branch `task/<task-id>` off current HEAD. Session cwd switches into it. ALL Phase 1-5 edits and commits happen here. Skip for `Trivial/Light` tasks where deploy isn't expected (worktree adds friction). Skip if user's invocation is `/leadv2 status`/`help`/`meeting`/`questions`/`sessions`. **Fork-owned session variant (FORK-RUNS-A-SESSION-01):** a fork never calls `EnterWorktree` (it shares the lead's CWD); the lead pre-creates the identical path/branch via `leadv2-fork-session.sh preflight <id>` and the fork works by absolute path under that lane root. **`preflight` must succeed or there is no fork** — it refuses (exit 1) when the isolated lane is unavailable and the lead runs the task itself; it never degrades to the shared checkout.
...
2. **Exit worktree (keep):** `ExitWorktree(action="keep")` — **lead calls this directly, never delegated to subagent.** Return to main repo dir, leave worktree on disk. Delegating ExitWorktree (or `git worktree remove`) to a subagent causes `ENOENT: posix_spawn '/bin/sh'` hook errors because the harness spawns hooks against a deleted CWD. A fork-owned session satisfies this ban by disjointness, not exception: it never entered a tool worktree (its lane was created by bash in preflight — which must have succeeded, or there is no fork), so Phase 6 lands via `leadv2-deploy-merge.sh` alone and reaping is the lead's `leadv2-fork-session.sh postflight <id>`. The fork's own commit goes through `leadv2-fork-session.sh commit <id>` (every git call carries `-C <lane-root>`); bare `git` from a fork is banned by mission text, not by hook.
