#!/usr/bin/env bash
# freepool-coder.sh v2 — headless GLM-5.3 code-worker wrapper + background workbench
# Live acceptance evidence: plugins/leadv2/docs/evidence/freepool-default-probe.md.
...
# under ~/.claude/cache/freepool-runs/<run-id>/ with a single-flight per-repo lock,
...
# sat silent for up to ~1h (the old FREEPOOL_TIMEOUT), indistinguishable from
...
# the whole detection primitive; rely on the existing FREEPOOL_TIMEOUT watchdog as
...
# FREEPOOL_TIMEOUT -> the existing timeout watchdog touches `.timed_out` FIRST,
# before TERM/KILL -> two-sentinel contract emits `FREEPOOL_FALLBACK_TO_SONNET` +
...
#   - FREEPOOL_FALLBACK_TO_SONNET / exit FREEPOOL_FALLBACK_EXIT_CODE (76): `.timed_out`
...
#   - FREEPOOL_PERMANENT_FAILURE: the child ran to completion with a COHERENT,
...
#     precedence).
# freepool-coder.sh does NOT invoke sonnet itself -- it only signals; the
# caller/Monitor decides what to do with each sentinel.
#
# FREEPOOL_CLAUDE_BIN/FREEPOOL_RUNS_DIR/FREEPOOL_SECRETS_FILE are test seams (default to the
...
readonly SECRETS_FILE="${FREEPOOL_SECRETS_FILE:-${HOME}/.claude/secrets/freepool.env}"
readonly FREEPOOL_BASE_URL="${FREEPOOL_PROXY_URL:-http://127.0.0.1:8317}"
readonly RUNS_DIR="${FREEPOOL_RUNS_DIR:-${HOME}/.claude/cache/freepool-runs}"
# FREEPOOL_TIMEOUT: wall-clock backstop. Turn and no-progress guards below stop
...
readonly FREEPOOL_TIMEOUT="${FREEPOOL_TIMEOUT:-3600}"
# FREEPOOL_STALL_S is retained as the stdout-idle observation threshold.  It is NOT
# a kill switch: a long tool call can legitimately leave stdout quiet.
readonly FREEPOOL_STALL_S="${FREEPOOL_STALL_S:-1200}"
readonly FREEPOOL_MAX_TURNS="${FREEPOOL_MAX_TURNS:-120}"
# Hard watchdog limits. FREEPOOL_MAX_TURNS remains the Claude CLI request limit and
# --max-turns remains backward compatible; FREEPOOL_TURN_LIMIT is independently
# enforced from observed stream turns through the timeout/fallback path.
readonly FREEPOOL_TURN_LIMIT="${FREEPOOL_TURN_LIMIT:-120}"
...
readonly FREEPOOL_NO_PROGRESS_S="${FREEPOOL_NO_PROGRESS_S:-1800}"
...
readonly FREEPOOL_FALLBACK_EXIT_CODE="${FREEPOOL_FALLBACK_EXIT_CODE:-80}"
readonly FREEPOOL_FALLBACK_SENTINEL="FREEPOOL_FALLBACK_TO_SONNET"
readonly FREEPOOL_CONTINUATION_MAX_LINES="${FREEPOOL_CONTINUATION_MAX_LINES:-200}"
...
# supervise or by hand, never through leadv2-dispatch-code.sh). freepool-coder.sh
...
usage() {
  cat >&2 <<'EOF'
Usage:
  freepool-coder.sh run "<prompt|@file>" [--out <file>] [--cwd <dir>]
  freepool-coder.sh bg  "<prompt|@file>" [--cwd <dir>] [--max-turns N] [--timeout S]
  freepool-coder.sh status [run-id]
  freepool-coder.sh tail <run-id>
  freepool-coder.sh watch <run-id>
  freepool-coder.sh list [N]
  freepool-coder.sh test
  ...
  bg      Detaches immediately, prints a run-id. Work continues under
          ~/.claude/cache/freepool-runs/<run-id>/ (journal.jsonl, progress.log,
...
Env knobs: FREEPOOL_TIMEOUT (default 3600s), FREEPOOL_MAX_TURNS (default 120),
  FREEPOOL_TURN_LIMIT (default 120), FREEPOOL_STALL_S (default 1200s, diagnostic only),
  FREEPOOL_NO_PROGRESS_S (default 1800s),
  FREEPOOL_FALLBACK_EXIT_CODE (default 76).
...
progress.log, mirrored into meta.yaml's exit_code field:
  FREEPOOL_FALLBACK_TO_SONNET   `.timed_out` fired, OR no coherent/non-error
                           ...
                           exit_code = FREEPOOL_FALLBACK_EXIT_CODE (76).
  FREEPOOL_PERMANENT_FAILURE    Coherent, non-error result but a genuine non-zero
...
freepool-coder.sh never invokes sonnet itself — callers/Monitor must react to
whichever sentinel appears.
EOF
}

# Loads FREEPOOL_AUTH_TOKEN from the secrets file into the current shell.
...
load_secret() {
  ...
  source "${SECRETS_FILE}"
  if [[ -z "${FREEPOOL_AUTH_TOKEN:-}" ]]; then
    log_error "FREEPOOL_AUTH_TOKEN is empty in ${SECRETS_FILE}"
    exit 1
  fi
}
...
run_claude() {
  ...
  (
    cd "${cwd_dir}"
    export ANTHROPIC_BASE_URL="${FREEPOOL_BASE_URL}"
    export ANTHROPIC_AUTH_TOKEN="${FREEPOOL_AUTH_TOKEN}"
    export ANTHROPIC_DEFAULT_OPUS_MODEL="freepool-default"
    export ANTHROPIC_DEFAULT_SONNET_MODEL="freepool-default"
...
cmd_run() {
  ...
  local out_file
  out_file="/tmp/freepool-coder-$(date '+%Y%m%d%H%M%S').out"
  local cwd_dir="${PWD}"
...
cmd_test() {
  local out_file
  out_file="/tmp/freepool-coder-test-$(date '+%Y%m%d%H%M%S').out"
  local exit_code=0
  ...
  run_claude "Reply with exactly: FREEPOOL-ALIVE <your model id>" "${out_file}" "${PWD}" 0 || exit_code=$?
...
acquire_lock() {
  ...
  if [[ -z "${lock_started}" ]]; then
    ...
    log_error "another freepool run is active for this repo (lock initializing, no started marker yet): ${lock_dir}"
  ...
  if [[ -n "${lock_pid}" ]] && ! kill -0 "${lock_pid}" 2>/dev/null; then
    ...
    log_error "another freepool run is active for this repo (lock: ${lock_dir}, pid: ${lock_pid:-unknown}). Use 'freepool-coder.sh list' to inspect."
...
endpoint: ${FREEPOOL_BASE_URL}
model: freepool-default
max_turns: ${max_turns}
timeout: ${timeout_s}
turn_limit: ${FREEPOOL_TURN_LIMIT}
no_progress_s: ${FREEPOOL_NO_PROGRESS_S}
pid: ${pid}
status: running
exit_code:
turns: 0
...
max_turns: ${max_turns}
timeout: ${timeout_s}
turn_limit: ${turn_limit:-${FREEPOOL_TURN_LIMIT}}
no_progress_s: ${no_progress_s:-${FREEPOOL_NO_PROGRESS_S}}
pid: ${pid}
status: ${status}
exit_code: ${exit_code}
turns: ${turns}
...
for raw in sys.stdin:
    ...
    try:
        if etype == "system" and ev.get("subtype") == "init":
            ...
            # the run is bounded solely by FREEPOOL_TIMEOUT.
            def _display_num(v):
                try:
                    return str(int(v))
                except Exception:
...
token = os.environ.get("FREEPOOL_AUTH_TOKEN", "")
...
cmd_run_child() {
  ...
  export ANTHROPIC_BASE_URL="${FREEPOOL_BASE_URL}"
  export ANTHROPIC_AUTH_TOKEN="${FREEPOOL_AUTH_TOKEN}"
  # FREEPOOL_AUTH_TOKEN itself must also be exported (not just as ANTHROPIC_AUTH_TOKEN)
  # so redact_stream()'s os.environ.get("FREEPOOL_AUTH_TOKEN") below actually sees it —
  # otherwise stderr redaction is dead code (R5 finding, GLM-ROUTING-V2-01 review).
  export FREEPOOL_AUTH_TOKEN
  export ANTHROPIC_DEFAULT_OPUS_MODEL="freepool-default"
  export ANTHROPIC_DEFAULT_SONNET_MODEL="freepool-default"
  export ANTHROPIC_DEFAULT_HAIKU_MODEL="freepool-default"
  export DISABLE_MODEL_AVAILABILITY_CHECK=1
...
append_continuation_block() { # <dead-run-dir> <next-prompt-file>
  ...
  max_lines="${FREEPOOL_CONTINUATION_MAX_LINES}"
...
watchdog_loop() {
  local child_pid="$1" timeout_s="$2" run_dir="$3" stall_s="${4:-${FREEPOOL_STALL_S}}"
  local turn_limit="${5:-${FREEPOOL_TURN_LIMIT}}" no_progress_s="${6:-${FREEPOOL_NO_PROGRESS_S}}"
  local waited=0 interval=2
  # FREEPOOL_STALL_S only reports stdout-idle.  FREEPOOL_NO_PROGRESS_S is the independent
...
cmd_supervise() {
  ...
  timeout_s="$(meta_get "${run_dir}" timeout)"
  [[ -n "${timeout_s}" ]] || timeout_s="${FREEPOOL_TIMEOUT}"
  ...
  ( watchdog_loop "${child_pid}" "${timeout_s}" "${run_dir}" "${FREEPOOL_STALL_S}" "${FREEPOOL_TURN_LIMIT}" "${FREEPOOL_NO_PROGRESS_S}" ) &
  local watchdog_pid=$!
  ...
  # FREEPOOL_FALLBACK_TO_SONNET sentinel path (no coherent result, no .timed_out).
  if [[ -f "${run_dir}/.stalled" ]]; then
    ...
    if [[ -z "${already_revived}" ]]; then
      ...
      # cmd_bg/cmd_run's freepool_launch_gate call, so a revive could silently
  ...
  if [[ "${exit_code}" -eq 0 ]] && [[ "${has_result}" -eq 1 ]]; then
    ...
    if [[ "${is_timeout}" -eq 1 ]] || [[ "${has_result}" -eq 0 ]]; then
      exit_code="${FREEPOOL_FALLBACK_EXIT_CODE}"
      echo "${FREEPOOL_FALLBACK_SENTINEL}" >> "${run_dir}/progress.log"
    else
      exit_code="${original_exit}"
      echo "${FREEPOOL_PERMANENT_FAILURE_SENTINEL}" >> "${run_dir}/progress.log"
    fi
  fi
...
cmd_bg() {
  ...
  local cwd_dir="${PWD}"
  local max_turns="${FREEPOOL_MAX_TURNS}"
  local timeout_s="${FREEPOOL_TIMEOUT}"
  ...
  [[ "${FREEPOOL_TURN_LIMIT}" =~ ^[1-9][0-9]*$ ]] || { log_error "FREEPOOL_TURN_LIMIT must be a positive integer"; exit 1; }
  [[ "${FREEPOOL_NO_PROGRESS_S}" =~ ^[1-9][0-9]*$ ]] || { log_error "FREEPOOL_NO_PROGRESS_S must be a positive integer"; exit 1; }
