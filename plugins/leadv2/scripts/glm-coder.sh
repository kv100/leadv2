...
deadhand_check() {
  ...
  if [[ -z "${path}" ]]; then
    ...
    # it. Measured live 2026-09-06: 259/281 (92.2%) of real GLM runs never
    # write .deliverable at all, so this branch WAS the population, not an
    # edge case. Three outcomes now, not two: real work landed (silent pass,
    # same as the declared-and-satisfied case below) / nothing landed (flagged,
    # reason=no_deliverable_declared) / could not be told apart (spoken aloud
    # via LEADV2_DEADHAND_UNVERIFIABLE, never silently folded into either).
