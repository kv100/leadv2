...
cmd_check() {
  ...
  while IFS=$'\t' read -r lane priority iid title; do
    ...
    # Keep the reason. Discarding it is how a hard failure (an arity drift in
    # the registry contract) spent its whole life looking like a soft capacity
    # refusal.
    _rl_err="$(_pump_reserve_lane "$iid" 2>&1 >/dev/null)"; _rl_rc=$?
    if [[ "$_rl_rc" -ne 0 ]]; then
      jemit decision "pump_skip task=${iid} reason=lane_reserve_failed rc=${_rl_rc} detail=$(printf '%s' "${_rl_err:-none}" | tr '\n' ' ' | cut -c1-200)"
