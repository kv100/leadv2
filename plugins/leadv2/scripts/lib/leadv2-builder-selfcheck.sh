...
#      LEADV2_TEST_FALSIFICATION_GATE (default 1, kill-switch for
#      TEST-FALSIFICATION-GATE-01, C4 below; 0 = zero-bookkeeping restore of the
#      pre-gate path, same idiom as LEADV2_SCOPE_DISCIPLINE).
#
...
lv2_selfcheck_run() {
  ...
  #
  # SELFCHECK-FORGED-MARKER-REGRESSION-01: this loop now also records WHICH paths the
  # diff CREATES (old side /dev/null). "The baseline has no copy of this file" splits
  # in two with opposite verdicts: a path that predates the lane in a tree whose
  # baseline cannot be materialised stays unattributable (fail-open, H1), but a path
  # this diff itself creates has no baseline copy BY CONSTRUCTION -- its red belongs
  # to the lane unconditionally. The diff file is generated by the caller (git diff
  # over the lane tree), never by the lane, so a test file cannot forge or strip its
  # own new-file header by anything it prints.
  local line path raw in_git_block=0 pending_new=0
  local new_paths="|" # pipe-delimited set: paths whose old diff side is /dev/null
  local -a changed=()
  while IFS= read -r line; do
    case "${line}" in
      'diff --git '*) in_git_block=1; continue ;;
      'new file mode'*) (( in_git_block )) && pending_new=1; continue ;;
      '+++ '*) raw="${line#+++ }" ;;
      '--- '*) raw="${line#--- }" ;;
      *) continue ;;
    esac
    ...
    # A pair always renders `--- old` then `+++ new`, so the old side answers "did the
    # lane create this path" one pair ahead of the new side that names it.
    [[ "${line}" == '--- '* ]] && { [[ "${raw}" == "/dev/null" ]] && pending_new=1 || pending_new=0; }
    ...
    changed+=("${path}")
    if [[ "${line}" == '+++ '* && "${pending_new}" == 1 ]]; then
      pending_new=0
      [[ "${new_paths}" != *"|${path}|"* ]] && new_paths="${new_paths}${path}|"
    fi
  done < "${diff_file}"
...
"
  ...
  _selfcheck_baseline_verdict() { # <suite_rel_path> -> prints PASS|FAIL|SKIP_RED|SKIP_UNRESOLVED
    ...
    if (( ! _baseline_ok )); then
      # SELFCHECK-FORGED-MARKER-REGRESSION-01: an unmaterialisable baseline must not
      # fail open for a file the lane itself created (new_paths, from the diff's
      # /dev/null old sides). Such a file has no baseline copy by construction, so
      # its red is attributable to exactly one party -- the lane -- and the gate
      # FAILs instead of excusing a forged marker. A path NOT created by this diff
      # keeps the fail-open: its red stays unattributable (H1).
      if [[ "${new_paths}" == *"|${rel}|"* ]]; then
        printf 'FAIL_NEW_FILE'
      else
    ...
    if [[ ! -f "${base_suite}" ]]; then
      # Same criterion, other entry point: the baseline tree resolved and still has
      # no copy of this path -- only a lane-created file can be absent from it.
      printf 'FAIL_NEW_FILE'
      return
    fi
    local blog brc
          ...
          if (( rc == 0 )); then
            ...
            case "${bverdict}" in
              ...
              SKIP_UNRESOLVED)
                ...
                ;;
              FAIL_NEW_FILE)
                _selfcheck_row "suites" "${test_rel}" "FAIL (no_baseline:file_new_in_diff)"
                failed=$((failed + 1))
                _selfcheck_fail_name "suites:test-${stem}$([[ ${rc} -eq 124 ]] && printf ':timeout')"
                _selfcheck_raw "${test_rel} (lane red, no baseline copy can exist -- the lane created this suite)" "${rc}" "${log}"
                ;;
              *)
                _selfcheck_row "suites" "${test_rel}" "${rc}"
                failed=$((failed + 1))
      ...
      if (( tf_rc != 0 )); then
        ...
        case "${tf_bverdict}" in
          ...
          SKIP_UNRESOLVED)
            ...
            ;;
          FAIL_NEW_FILE)
            # SELFCHECK-FORGED-MARKER-REGRESSION-01: the lane created this test file,
            # so no baseline copy can exist to inherit redness from -- a red rc here
            # is the lane's own failure and blocks even when the baseline tree is
            # unmaterialisable (which is where the forged marker used to slip through).
            failed=$((failed + 1))
            _selfcheck_row "falsification" "${tf_rel}" "FAIL (test_failed:rc=${tf_rc};file_new_in_diff)"
