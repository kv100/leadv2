...
def ecost(c):
    ...
    # MERGE 2026-09-13: two lanes landed on ecost in the same session and each
    # replaced a different half of this expression. cc4557feef48 made the
    # numerator a PER-PROVIDER price (provider_cost) instead of the per-model
    # `cost` column; 48b8297b4cc1 added reset_urgency_weight to the denominator
    # so quota about to be thrown away is preferred. They are independent terms
    # and both are kept -- taking either side alone silently deletes a landed,
    # suite-proven behaviour.
    _ru=reset_urgency_weight(c.get('provider'), c.get('arm'))
    _base=provider_cost(c)/((_w if _w > 0 else 1.0)*_ru)
