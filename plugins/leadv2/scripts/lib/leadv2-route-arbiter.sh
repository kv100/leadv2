...
print('arm=%s model=%s tier=%s reason=cheapest_capable chain=%s %s%s%s%s%s' % (w['arm'],w['model'],w.get('tier','standard'),','.join(rotated),ufmt(),_extra,_floor,_fmode,_complexity))
PY
}
