...
_dumprows() { python3 -c 'import sys,yaml; d=yaml.safe_load(open(sys.argv[1])) or {}; print("DUMP:", [ (x.get("task_id"), x.get("pid")) for x in (d.get("sessions") or []) ])' "$ACTIVE_YAML"; }
rm -rf /tmp/yaml-snaps; mkdir -p /tmp/yaml-snaps; rm -f /tmp/stop
( i=0; while [ ! -f /tmp/stop ]; do i=$((i+1)); cp "${ACTIVE_YAML}" "/tmp/yaml-snaps/s$i.yaml" 2>/dev/null; sleep 0.05; done ) & _W=$!
_rows_before="$(_dispatch_rows)"; _dumprows
...
( cd "$REPO" && LEADV2_DISPATCH_ARCHITECT_GATE=1 \
  ...
  BASH_XTRACEFD=7 bash -x "$DISPATCH_SH" 'e2e park must release the registered slot' --kind product --protected \
  --writes "a.txt,b.txt,c.txt" > "$ROOT/e2e-out.log" 2>&1 7> "$ROOT/e2e-xtrace.log" ) || rc_e2e=$?
...
touch /tmp/stop; kill $_W 2>/dev/null; _rows_after="$(_dispatch_rows)"; _dumprows
